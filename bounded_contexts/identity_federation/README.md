...
2. 無ければ**検証済みの**メールアドレスで既存のローカルアカウントへ寄せる
   （⚠ **`OIDC_LINK_BY_EMAIL` が真のときだけ。既定は偽**。ADR-0037）
...
⚠ **`email_verified` の意味は IdP ごとに違う。** 自前 idp (assay) でこの値が真に
なる経路は自己登録の確認メールだけで、**管理者によるメール変更はこの値を維持する**。
つまり「本人が所有を証明した」ではなく**「テナント管理者がそう主張している」**である。
2 を開けるなら、つないだ IdP でこの値がどう立つのかまで確かめること（ADR-0037）。
