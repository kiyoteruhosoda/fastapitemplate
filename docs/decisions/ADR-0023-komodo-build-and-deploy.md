...
- 日付: 2026-08-24
- 状態: 廃止（ADR-0044 で置き換え）

> [!WARNING]
> **Komodo は 2026-09-08 に停止した。デプロイ先は k3s である**（ADR-0044）。
> 以下は決めたときの記録として残す。⚠ **Komodo を起こさないこと**
> ——compose 側が k3s と同じデータを掴みに行く。
>
> ADR-0044 が引き継いだのは「成果物はコンテナイメージ 1 つ」「版の刻印は
> ビルドの前に行う」「サービス名に一般名を使わない」の 3 つで、
> **配り方（Build / Stack / ResourceSync / Variable）はすべて置き換わった。**
