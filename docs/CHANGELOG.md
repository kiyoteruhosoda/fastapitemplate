## 2026-09-23（版はコミットの短縮ハッシュ 1 つ。`v` も括弧も付けない）

- ⚠ **フッターに `vv11eeceb (11eeceb)` と出ていた。** `v` が 2 つ付き、同じ値が
  2 回並んでいた。出どころは 2 か所ある。
- **`scripts/generate_version.sh` が `v` を付けるのをやめた**（`VERSION="$COMMIT_HASH"`）。
  意味のある版数（`v1.2.3`）ではなくコミットの短縮ハッシュなので、`v` があると
  「そういう版がある」と読めてしまう。
- **画面（`Footer.tsx`）も `v` を足すのをやめ、`git_sha` を括弧で添えるのもやめた。**
  版がコミットの短縮ハッシュになった以上、同じ値が 2 回並ぶだけだった。枝で焼いた
  ときは版のほうに枝の名前が付くので、出すなら版 1 つでよい。
- nolumiawiki は 2026-09-06 に、nolumiatask は 2026-09-23 に同じ形へ直してある。

## 2026-09-21（Komodo 前提の文書を k3s の実態へ揃えた。ADR-0044）

- ⚠ **Komodo は 2026-09-08 に停止していたのに、文書がそのままだった。** README・CLAUDE.md・
  OPERATIONS.md・`.env.example`・`Dockerfile`・`scripts/` が「Komodo Build が焼く」
  「秘密は Komodo の Variable」「`stacks.toml` にスタックを足す」と説明し続けており、
  **ここから作ったアプリを載せる手順として 1 つも通らなかった**。
- **デプロイ先は k3s、押す口は deck**（`https://deck.nolumia.com`）に書き換えた。段は
  **build → pin → deploy**。稼働状態の正は deploy-repo の `k8s/<app>-<env>/`。
- **`deploy/komodo/` を `deploy/k8s/` に置き換えた。** compose・`build.toml`・`stack.toml` を
  消し、宣言一式（namespace / PV・PVC / ConfigMap / MariaDB / app・front ＋ NodePort）の
  雛形を置いた。秘密は**封印して** `25-secrets.yaml` に置く（平文の `kind: Secret` は CI が落とす）。
- ⚠ **`docker/nginx/default.conf.template` は共有ではなくなった。** デプロイ先の nginx は
  ConfigMap で、**resolver が違う**（`127.0.0.11` → CoreDNS の `10.43.0.10`）。片方を直したら
  もう片方も直す旨を両側に書いた。
- ⚠ **`init-paths` は要らなくなった**（所有者合わせは `fsGroup`）。⚠ **`replicas` は宣言に
  書かない**（台数の正本は deck の scale。書くと `apply` が止めてある環境を起こす）。
- **成果物の置き場とタグが変わった。** `hub.nolumia.com:5000/komodo/<app>`（`latest` /
  `<コミット>` / `0.0.N`）→ **`hub.nolumia.com:5000/app/<image>`（`sha-<コミット>`）**。
  ⚠ **版は digest で固定する** ——`:latest` だと宣言が変わらず `kubectl apply` でも
  rollout が起きない。戻すのは前の digest に戻すこと（タグはローテされるが digest は残る）。
- ADR-0023 を**廃止**にし（ADR-0044 で置き換え）、引き継ぐ 3 点（成果物はイメージ 1 つ／
  版の刻印はビルドの前／サービス名に一般名を使わない）を新しい ADR に明記した。
- ついでに 2 つ直した: `.github/dependabot.yml` を置くという T8 の記述（**forge では
  動かない**。版上げは Renovate が既に見ている）と、ソースの正本が GitHub だという記述
  （**forge が正本**で GitHub は押し出しミラー）。

## 2026-09-16（IdP で止まった人を、毎時聞き直して止める。ADR-0043）

- ⚠ **停止の通知（ADR-0036）は、届いたときにしか効かなかった。** assay が送れない・こちらが
  落ちていた・送り先が未登録、のどれでも取りこぼしがそのまま残っていた（idp の ADR-0049 I7 の 3 つ目。
  旧 T12）。**毎時 assay の名簿（`/admin/applications/self/users`）を聞き直す**ようにした。
- **止めるのは SSO で始まったセッションだけ。** 停止の通知と同じ記録（`federated_session_revocations`）
  を書く。⚠ **`is_active` は落とさない** ——この雛形はローカル口座を意図して持つ（ADR-0039）。
  先に入れた nolumiawiki は利用者ごと止めており、assay で戻しても戻らなかった。
- `unknown`（消えた）は結び付きも外す。⚠ **全員が `unknown` なら何も変えない**（テナントの取り違え）。
- ⚠ **設定 `MACHINE_CLIENT_ID` が増えた。** 空なら走らない。assay 側でサービスアカウントの登録と、
  アプリの「名乗り」への結び付けが要る（`docs/OPERATIONS.md`）。

## 2026-09-15（写しは IdP を正とし、メールアドレスを鍵扱いするのをやめた。ADR-0042）

- ⚠ **プロフィールの写しが初回以降まったく更新されていなかった**（idp の ADR-0049 G4）。
  `provision` した一度だけ書かれ、IdP で改名・メール変更をしても**こちらの表示は
  永久に古いまま**だった。**ログインのたびに上書きする**ようにした。
  ⚠ **ぶつかる値は書かない** ——一意の列なので、そのまま書くとログインが 500 で落ちる。
- ⚠ **メールアドレスが無いと弾いていた**（G5）。鍵は `(iss, sub)` だと決めているのに、
  必須にしていたのは**鍵にしていた名残**である。**要るところでだけ要る**ようにした
  ——初回に寄せるときと、利用者を作るときだけ。
- ⚠ **`groups` からロールを引く設定が、一度も効いていなかった**（G7）。assay は
  `groups` を発行しない。**効かない設定は、効くと思って触られる**ので消した
  （`OIDC_GROUPS_CLAIM` / `OIDC_ROLE_MAPPING`）。
- ⚠ **`OIDC_ROLE_SYNC` の意味が狭くなった。** 引き直すのは既定のロールだけなので、
  真にすると**管理画面で足したロールが毎回のログインで消える**。

## 2026-09-15（アクセストークンの検証から DB を外し、寿命を 5 分にした。ADR-0041）

- ⚠ **認証済みの全リクエストが DB を引いていた** ——`users` の 1 件引き、ロールと権限の
  遅延読み込み（ロールの数だけ）、失効の記録の照会。アプリのいちばん内側で最も多く
  走るクエリだった。
- **検証を署名とクレームだけにした。** `verify_access_token_with_reason` から
  `Session` を外し、`get_current_principal` からも DB の依存を外した。
  ⚠ **`username` をアクセストークンへ足した**（検証が引かない以上、principal の材料は
  全部クレームに載せる。載せ忘れるとその経路でだけ静かに空になる）。
- **寿命を 900 → 300 秒にした。** ⚠ **この値が「止めたら効くまで」の上限時間そのもの
  になる。**
- ⚠ **止める判定は更新の 1 点に集約した。** `verify_refresh_token` は DB を引き続ける。
  **ここが引き締めているからアクセス側を緩められる** ——緩めると上限そのものが消える。
- ⚠ **新しい入り口を作る経路（二要素・パスキー）も DB を引く。** 止められた相手に
  入り口を増やされては、止めた意味が半分無くなる。**規則は 3 つ** ——読むだけは
  引かない / 新しいトークンを出すなら引く / 新しい入り口を作るなら引く。
- ⚠ **引き受けた変化**: 停止（ローカルの無効化・IdP からの停止）も、ロールの取り上げ
  （ADR-0017）も、**最大 5 分遅れて効く**。`test_the_access_token_outlives_the_stop_only_until_it_expires`
  がこの緩さを明示的に固定している。
- **更新の回数は 3 倍になる**（15 分 → 5 分）。DB の仕事は消えるのではなく更新へ移る。

## 2026-09-14（本人の操作で IdP の口座と結び付けられるようにした。ADR-0040）

- ⚠ **ADR-0037 で自動リンクを倒して以来、逃げ道が 2 つしか無かった** ——設定を
  開け直すか、管理者が `federated_identities` を手で整えるか。どちらも「なぜ倒したか」
  を戻すか、手作業を運用に残すかになる（idp の ADR-0049 I9）。
- **設定画面（`/profile/security`）から連携できるようにした。** 結び付けてよいかは
  **2 つの認証の結果**で決まる ——いまこのアプリのセッションを持っていることと、
  いま IdP の認可の往復を通ってきたこと。**両方を同時に満たすのは本人だけ**なので、
  メールアドレスという「同じ値かもしれないもの」を判断に使わない。
- **戻り先の URI は増やしていない。** `GET /api/auth/sso/callback` がログインと連携の
  両方を受け、どちらの後始末をするかは往復状態の `purpose` で決める。⚠ **目的を
  クエリで渡さない** ——URL を書き換えるだけで化けさせられる。
- ⚠ **結び付ける相手は「往復を始めた利用者」で、戻ってきたときのセッションではない。**
  往復状態に `user_id` を入れて突き合わせる。これが無いと、途中で入れ替わった
  ブラウザが**別人の口座へ**結び付ける。
- ⚠ **既に別の利用者のものになっている `(issuer, subject)` は断る**（横取りになる）。
  同じ IdP の別の口座への差し替えも断る（前の入り口が予告なく消えるため）。
- ⚠ **解除は入り口が残るときだけ通す。** 数えるのはパスワードとパスキーで、
  **二要素認証は数えない**（パスワードの後ろに置くもので、それだけでは入れない）。
- ⚠ **往復状態の Cookie の形が変わった。** 移行前に発行された Cookie は `purpose` を
  持たないので、ログインの往復として読む（連携が入る前の往復は必ずログインである）。

## 2026-09-14（「この利用者が入れる手段」を数えられるようにした。ADR-0039）

- ⚠ **このアプリには認証系が 2 つある**（IdP 側とこのアプリ側）。IdP で多要素を
  必須にしても、こちらのローカルログインには掛からない。**そして「利用者が入れる
  手段」の一覧がどこにも無かった**（idp の ADR-0049 G1）。
- **この雛形は「ローカル口座を意図して持つ」と決めた**（ADR-0039）。雛形の用途が
  「IdP が無くても動くアプリの出発点」である以上、SSO 専用にはできない。
  ⚠ **雛形から作ったアプリは、作った時点でどちらかを自分の ADR に書くこと。**
- **`GET /api/admin/users` に `entrances` を足した** ——パスワード / 二要素認証 /
  パスキーの本数 / 結び付いている IdP。材料はそれぞれのコンテキストに聞いている。
- **ユーザー管理の画面に「入れる手段」の列が出る。** 表示だけで、ここからは
  取り上げられない（取り上げ方はそれぞれの口の側にある）。
- ⚠ **IdP 側の多要素はこの一覧に出ない。** 出せるのはこのアプリが知っている口だけで、
  **混ぜられないことこそが「認証系が 2 つある」の実体**である。
- ⚠ **多要素の強制はまだ無い**（`Progress.md` の T14）。

## 2026-09-14（「ローカルのパスワードが無い」をデータで表せるようにした。ADR-0038）

- ⚠ **SSO で作った利用者に、誰も知らないランダム値を入れていた**（`password_hash` が
  `NOT NULL` だったため）。動きはするが、**「パスワードが無い」と「誰も知らない
  パスワードがある」が区別できない**（idp の ADR-0049 I4 / G2）。
- **`users.password_hash` を NULL 許容にし、NULL =「ローカル認証を持たない」と定めた。**
  判定は `User.has_local_password` に集めた。
- ⚠ **いちばんの中身はパスワードリセットを塞いだこと。** それまでは、SSO でしか
  入れないはずの利用者が**リセットを 1 回通すだけでパスワードでも入れる**ように
  なっていた——誰も操作していないのに入り口が 1 つ増える。応答は宛先が無いときと
  同じに揃えてある（存在を漏らさない）。
- **画面はパスワード変更のフォームを出さない**（`/api/auth/me` の `has_password`）。
  出すと、「今のパスワード」を入力できない相手に絶対に通らない入力欄を見せることになる。
- **持たせるのは管理者が明示的にパスワードを設定したときだけ。**
- ⚠ **移行が既存のランダム値を落とす。** どの行がそれかは値からは分からないので、
  **利用者の行と IdP との結び付きが 60 秒以内に作られたか**で判断する。取り違えても
  「入れなくなる」（気付ける）だけで、「入り口が増える」（気付けない）ことはない。
  ⚠ **知らない方言の DB では移行が何もしない**（手当ては `OPERATIONS.md`）。

## 2026-09-14（同じメールアドレスの口座へ黙って寄せなくなった。ADR-0037）

- ⚠ **`OIDC_LINK_BY_EMAIL` の既定を `true` から `false` へ倒した。** 初回の SSO
  ログインで、同じメールアドレスのローカル口座へ**自動で寄せることをやめた**。
- ⚠ **条件だった `email_verified` の意味が、IdP 側と食い違っていた。** RP は
  「本人が所有を証明した」と読んでいたが、自前 idp (assay) でこの値が真になる経路は
  自己登録の確認メールだけで、**管理者によるメール変更はこの値を維持する**。つまり
  **「テナント管理者がそう主張している」**でしかなく、付け替えたアドレス 1 つで
  既存の口座へ入れる経路になっていた（idp の ADR-0049 G6）。
- **既に結び付いている利用者には影響しない**（`federated_identities` に行があれば
  そちらで決まる）。効くのは**これから初めて SSO で入る人**だけ。
- これまでの挙動が要るなら `OIDC_LINK_BY_EMAIL=true` を明示する。⚠ **そのとき、
  つないだ IdP で `email_verified` がどう立つのかを確かめること。**
- 本人の操作で結び付ける導線（本筋）は ADR-0040 で入った。

## 2026-09-14（IdP で止めた利用者が、このアプリでも止まるようになった。ADR-0036）

- ⚠ **これまで、IdP で利用者を止めても RP のセッションは寿命まで生きていた。**
  idp は back-channel logout の送り手（再送つきの配信キュー）を持っているのに、
  **受け口を実装した RP が 1 本も無かった**（idp の ADR-0049 G3）。
- **`POST /api/auth/sso/backchannel-logout` を足した。設定は要らない**（常に受ける）。
  栓になるのは IdP 側の `backchannel_logout_uri` の登録だけ。
- ⚠ **セッションを消すのではなく「止まった」を記録する。** トークンはサーバーに
  控えの無い JWT なので、消せるものが手元に無い。`federated_session_revocations` に
  1 行残し、**次にトークンが提示されたときに弾く**。
- ⚠ **比べるのはトークンの `iat` ではなく、セッションが始まった時刻**（`fed_iat`）。
  `iat` は更新のたびに新しくなるので、**更新を 1 回通すだけで停止をすり抜けられた**。
  秒までしか持たないため、止めた直後の同じ秒に入り直した利用者も巻き添えにしていた。
- ⚠ **`nonce` を持つ通知は断る。** logout token と ID トークンは署名鍵も発行者も
  対象者も同じで、**両者を分けるのはこの 1 点だけ**。受け付けると、正規に受け取った
  ID トークンを送り返すだけで他人のセッションを落とせる。
- **再送は `jti` で弾く**（主キーにした）。弾かないと、古い通知の再送で
  「いま生きているセッション」を落とせる。
- ⚠ **これだけでは「止めたら届く」は完成しない。** 残り 2 つ（短命のアクセストークン・
  定期照合）は `Progress.md` の T12。**いまの assay は「利用者を止めた」ときに通知を
  送らない**（送るのはログアウトのとき）。

## 2026-09-14（新しい版が出たことを画面で知らせる。ADR-0035）

- ⚠ **デプロイしても、開きっぱなしの画面は古い版のままだった。** SPA は画面を
  描き替えるだけで読み込み直さないので、直した不具合がその端末ではそのまま見えて
  いた。**インストールした PWA は特にそう**で、タブを閉じるまで何日でも古い版が動く。
- **`registerType` を `autoUpdate` から `prompt` へ**（`frontend/vite.config.ts`）。
  ⚠ `autoUpdate` は新しい Service Worker を黙って有効にするだけで、
  **既に読み込まれた JavaScript は差し替わらない**（＝画面は古いまま）。
- **「新しい版があります。[再読み込み]」を画面の下に出す。** 押されるまで消えず、
  押したときだけ入れ替わる（書きかけの入力を奪わない）。
- ⚠ **更新の確認を自分で起こすようにした。** ブラウザが確認するのはページ遷移の
  ときで、**SPA の画面切り替えはページ遷移ではない**。30 分ごと・タブが手前へ
  戻ったときに確認する（オフライン・直前に確認済みなら起こさない）。
- ⚠ **押しても何も起きない場合の逃げ道を入れた。** その画面をまだ Service Worker が
  制御していない（登録された直後の初回訪問）と、制御が移る合図が来ないまま
  止まる。2 秒待ってこちらからも読み直す。実ブラウザ（Playwright）で
  制御あり・なしの両方を通して確認した。

## 2026-09-10（画面で保存した設定が環境変数より強くなった。ADR-0034）

- ⚠ **解決順を `DB > 環境変数 > デフォルト値` へ入れ替えた。** それまでは
  環境変数が最優先で、**compose に 1 行書いた鍵は管理画面から触れなかった**
  （入力欄が `env_locked` で無効化されていた）。派生アプリで実際に踏んだ
  ——画面に出ていて、値も見えていて、変えられない。
  **環境変数は「配るときの初期値」**、**運用中に決め直す場所は画面**、に揃えた。
- **`env_locked` を `env_fallback` に変えた。** 止める印ではなく、
  「この鍵は環境変数にも値がある＝画面の値を消すとそちらへ戻る」という案内。
  入力欄はもう無効化しない。⚠ **消したときの戻り先は既定値ではなく環境変数**である。
- ⚠ **締め出しからの戻り道が変わった。** `LOCAL_LOGIN_ENABLED=false` で
  締め出されたときの復旧は、環境変数ではなく **保存を 1 鍵だけ消す**
  （`scripts/unset_setting.py`。`docs/OPERATIONS.md` に手順）。
  ⚠ **`system_settings` の行ごと消さない** ——1 行に全部の鍵が入っている。
- ⚠ **入れ替えは、いま DB に保存されている値の意味を変える。** 配る前に
  「DB に在る鍵」と「環境変数に在る鍵」の重なりを見ること。

## 2026-09-07（2）（RP ログアウトの残り 3 点。ADR-0033 に追記）

- ⚠ **`GET /api/auth/sso/logout` によそから呼ばれる余地があった。** 未認証・GET・
  状態変更で、しかも変えるのは**全アプリで共有された IdP の SSO セッション**。
  よその頁が `<img src="…/api/auth/sso/logout">` を置くだけで、**利用者を他アプリ
  からも締め出せた**（被害はログインし直しまで）。`Sec-Fetch-Site` を見て、
  自分のオリジンからの遷移と利用者自身の操作だけ通すようにした。
  ⚠ **IdP の確認画面はあてにできない**——OIDC は *SHOULD* としているだけで、
  自前 idp (assay) は挟まない（実装を確認した）。**ヘッダーごと無い場合は通す**
  （付けないのは古いブラウザ。よその頁からの遷移では今のブラウザが必ず付ける）。
- **判断材料を `/api/auth/sso/provider` から `/api/auth/me` へ移した。** 画面が
  サインアウトのたびに `/provider` を引いていて、**SSO を使わない構成でも毎回
  往復が 1 つ増えていた**。`/me` は起動時に必ず引くので、載せるぶんには増えない。
  ⚠ `/me` が返すのは**設定であって、この画面がどう入ったかではない**（理由は ADR）。
- **`?signed_out=1` を画面が読んでいなかった。** IdP でサインアウトを終えて戻っても
  ただのログイン画面が出るだけで、**期限切れで飛ばされたのと見分けが付かなかった**。
  `login.signedOut` を出す（ja / en）。

## 2026-09-07（サインアウトを IdP まで通せるようにした。**既定は無効**。ADR-0033）

- **サインアウトが IdP に届いていなかった。** `POST /api/auth/logout` はこのアプリの
  セッションを終わらせるだけなので、SSO で入った利用者がサインアウトしたあと
  「SSO で入る」を押すと**名乗り直さずに入れる**。自前 idp を使う SSO 専用のアプリでは
  これが「サインアウトしてもサインアウトされない」という形で表に出た。
- **通す道を用意した。既定は無効**（`OIDC_RP_LOGOUT_ENABLED`。ADR-0033）。
  有効のときだけ、画面はアプリのサインアウトを終えてから `GET /api/auth/sso/logout`
  へ遷移し、そこが IdP の `end_session_endpoint` へ送り出す。戻り先は
  `GET /api/auth/sso/signed-out`（`OIDC_POST_LOGOUT_REDIRECT_URI` が空なら
  `APP_BASE_URL` から組み立てる）。
- ⚠ **既定を無効にしたのは副作用があるため。** 消えるのは **IdP の SSO セッション**で、
  これはその IdP を使う全アプリで共有されている。片方のサインアウトが、もう片方の
  **次回 SSO ログイン**に効く。**ローカル口座を持つアプリは無効のままでよい。**
  有効にする価値があるのは SSO でしか入れないアプリと、共有の端末で開くもの。
- ⚠ **有効にしたら `post_logout_redirect_uri` を IdP へ登録すること。** 未登録でも
  壊れはせず **IdP 自身の完了ページで止まる**ので、登録漏れは「戻ってこない」という
  形でしか出ない（手順は OPERATIONS）。
- **通せない場面は失敗にしない。** IdP が `end_session_endpoint` を出していない・
  discovery が引けない・SSO が使えない——どれもログイン画面へ返すだけにする。
- **`id_token_hint` は送っていない。** この雛形は検証後の ID トークンを保持しないため。
  `client_id` + `post_logout_redirect_uri` の組で名乗る。必要になったら行を増やす話。

## 2026-09-05（名前の直書きと、SPA が API を飲み込む問題を直した。ADR-0031 / ADR-0032）

- **`USER_AGENT` に `"nolumiawiki"` が直に書いてあった。** 派生アプリで入れた修正を
  テンプレートへ戻すときに値ごと持ち帰ったもので、**この行を直さないかぎり、
  あらゆる派生アプリが IdP に対して別のアプリの名前で名乗り続ける**状態だった
  （2026-09-05 に `mp3play` を起こしたときに気付いた）。`FastAPI(title=...)` も同じく
  `"fastapitemplate"` が直書きだった。
- **名前の正本を `pyproject.toml` の `[project].name` 1 か所にした**（ADR-0031）。
  `shared/kernel/version.py` に `project_name()` を置き、`BuildInfo.name` として運ぶ。
  Swagger の題と `User-Agent`（`<name>/<version>`）はここから導く。
  ⚠ **読めなかったときの既定は `"app"`。** テンプレート名にすると、読めていないことに
  気付けなくなる。
- **SPA の受け皿が `/api/` を飲み込んでいた**（ADR-0032）。存在しない API のパスが
  **`200` + `index.html`** を返していた。しかも `frontend/dist` があるときだけなので、
  **本番でだけ起きて手元と CI では再現しない**。受け皿に落ちた `/api/` は 404 にした
  （形は dist の無いときと同じ）。
- ⚠ **どちらも派生アプリ側の直書きは自動では消えない。** `git merge template/main` を
  取り込んだら、`app.py` と `oidc_metadata.py` に自分で書いた名前が残っていないか
  確かめること。

## 2026-09-04（4）（GitHub Actions を Node 24 で動く版に上げた。ADR-0030）

- **CI の注釈に `Node.js 20 is deprecated` が出ていた。** ランナーが Node 20 を
  落としたので、`node20` を要求する action は Node 24 で**強制的に**動かされて
  いた。互換のための猶予なので、切れたら止まる。**チェックは緑のままで、
  注釈にしか出ない。**
- `astral-sh/setup-uv` `@v6` → **`@v10.0.1`**、`docker/setup-buildx-action`
  `@v3` → `@v4`、`docker/build-push-action` `@v6` → `@v7`。これで
  `node20` を要求する action は無くなった（`actions/checkout@v6` と
  `actions/setup-node@v6` は元から `node24`）。
- ⚠ **`setup-uv` だけ完全な版で固定してある。** 上流が v8 以降、動く major タグ
  （`@v10` 等）の発行をやめたため（タグの差し替えによるサプライチェーン攻撃を
  避けるため）。**書き方が揃っていないのは意図的**なので、`@v10` へ「直さない」こと。
  上げるときは版を書き換える。
- `setup-uv` v9 の破壊的変更で **`prune-cache` の既定が `false`** になった
  （PyPI の負荷を下げるため）。Actions のキャッシュ使用量は増えるが上限内で
  回るだけなので、設定は足していない。
- ⚠ **派生アプリがセルフホストのランナーを使っている場合は先にランナーを更新する。**
  docker の 2 つは Actions Runner v2.327.1 以降を要求する。

## 2026-09-04（3）（マイグレーションが初期管理者を据えないことを機械で見張る）

- **`tests/unit/test_migrations_do_not_seat_an_admin.py` を足した。** ADR-0024 が
  決めた「`ensure_default_admin` を呼ぶのは据え付けの `0002` と投入スクリプトだけ」
  を、`migrations/versions/` を AST で走査して検証する。破ったマイグレーションを
  **追加した時点で**落ちる（当てたあとではない）。
- **既存の回帰テストでは足りなかった。** `tests/integration/test_master_data_seeder.py`
  が見張るのは `seed_master_data` の振る舞いなので、新しいマイグレーションが
  `ensure_default_admin` を自分で呼ぶ形は素通りしていた。規約（CLAUDE.md）と
  レビューだけが歯止めだった。
- **関数を呼ばない形も対象。** `master_data.DEFAULT_ADMIN_PASSWORD_HASH` 等を直に
  持ち出して自前で `User` を組み立てる書き方も同じ地雷なので拾う。`import` の
  別名付け（`as`）と属性参照でもすり抜けない。
- 許可リスト（`0002_seed_master_data.py`）が空振りしていないことも併せて確認する。
  据え付けの経路が別のファイルへ移ったら落ちる。

## 2026-09-04（2）（トークンを Cookie 一本にした。ADR-0028）

- ⚠ **API の互換が壊れる。** ログイン・更新・ロール切り替え・SSO の引き換えは、
  応答本文にトークンを載せなくなった（`expires_in` だけを返す）。**本文から
  トークンを取り出している機械の呼び出し元は直す必要がある**（`Set-Cookie` を
  保持して以後 Cookie で叩く）。ブラウザからのログインは壊れない。
- **SPA はトークンを持たない。** localStorage から消し、ログイン済みの判定は
  `GET /api/auth/me` の成否に寄せた。access / refresh とも httpOnly Cookie。
- **本文に載せないのが肝。** Cookie は自動で送られるので、本文にも返すと XSS が
  `POST /api/auth/refresh` を叩いて新しいトークンを読めてしまい、httpOnly にした
  意味が無くなる。`Authorization` ヘッダーは引き続き受け付ける（curl・CI・他アプリ
  のため）——**危ないのはヘッダーを受け付けることではなく、JS から読める場所に
  置くこと**。
- **CSRF の二重送信トークンを足した。** Cookie で認証した更新系にだけ求める。
  読み取りと、`Authorization` ヘッダーで来た呼び出しには求めない（ヘッダーは自動では
  送られないので CSRF が成立しない）。⚠ **入口（ログイン・パスキー・SSO の引き換え・
  パスワード再設定・ログアウト）は対象外** ——ここで求めると、別のアカウントで
  ログインし直すだけで 403 になる。
- `POST /api/auth/refresh` は本文でトークンを受け取らなくなった（Cookie から読む）。
- ⚠ **テストは 1 クライアント = 1 セッションになった。** 管理者と一般利用者を行き来する
  テストは `other_client` フィクスチャで分ける（ブラウザと同じ）。

## 2026-09-04（外部 IdP との SSO を足した。ADR-0025 / ADR-0026）

- **`bounded_contexts/identity_federation/` を追加した。** 認可コードフロー + PKCE で
  外部 IdP へログインを委譲する。経路は `/api/auth/sso/{provider,login,callback,token}`。
  クライアント認証は `client_secret_basic` と `private_key_jwt` の両対応で、
  **既定は無効**（`OIDC_ENABLED=false`）。派生 3 つ（nolumiawiki / rewardpointsweb /
  photonest）が別々に書いていたものを、部分ごとに良い方を採って持ってきた。
- **往復状態（`state` / `nonce` / `code_verifier`）は署名付き Cookie で運ぶ。** 表に
  控えを置かないので、`state` を知っているだけの相手は戻りを完了できない
  （ログイン CSRF）。テーブルは 2 つだけで、掃除も要らない。
- **コールバックは 1 回限りの引き換え券だけを URL に載せる。** トークンは URL に
  載らない（履歴・Referer・プロキシのログに残るため）。
- **要求した認証の強度は確かめる。** `OIDC_ACR_VALUES` を送ったら、返ってきた `acr` が
  要求と一致しなければ（返ってこない場合も）ログインを断る。既定は空なので、
  設定した派生だけが fail closed になる。
- **`OIDC_AUTO_PROVISION` の既定は `false`。** IdP がテナント共用のとき、`true` は
  「IdP に口座がある人は全員このアプリに入れる」を意味する。
- **`LOCAL_LOGIN_ENABLED` を足した。** `false` でパスワード・パスキーのログインと、
  ローカル資格情報の登録（パスキー・TOTP・パスワード変更）を止める。登録まで止める
  のは、閉じた入口の合鍵を作れないようにするため。⚠ 締め出されたら環境変数で
  `true` へ戻して再起動する。
- ログイン成功の監査ログに**どの入口で入ったか**を残すようにした
  （`method=password` / `passkey` / `sso`）。

# CHANGELOG — 完了した重要な変更の要約

## 2026-09-04 入力の面を整理し、スマートフォンでは幅いっぱいに出すようにした

390px 幅では、どの管理画面も**最初の 1 画面が入力欄で埋まり、その画面が扱うデータが
下へ押し出されていた**（監査ログは 1 件目まで約 1,300px スクロール）。方針は
**ADR-0027**。

- **追加フォームを `<dialog>` へ移した**（アイテム・ユーザー・ロール）。画面には
  「追加」ボタンだけを置き、一覧が先に来る。狭い画面では画面の下から**幅いっぱい**の
  シートとして開き、広い画面では中央に開く。どちらで出すかは CSS が決める
  （コンポーネントは幅を判定しない。ADR-0011 を引き継ぐ）。
- **ログの絞り込みを狭い画面で畳めるようにした。** 広い画面は今までどおり常に開いた
  状態（開閉ボタンごと `display: none`）。
- **権限・ロールの対応表を読めるようにした。** 列見出しを縦組みにし、行の見出しを
  左端に固定し、カードの幅の上限を外した。監査ログの 11 列は 1280px 幅で
  横スクロールが消え、390px 幅では見える列が 2 → 4 に増えた。
  ⚠ `RolesPage` は以前から `<th className="vertical">` を指定していたが、
  **`index.css` に `.vertical` の定義が無く、何も起きていなかった**。
- 併せて、狭い画面の細かな読みにくさを直した。設定名と環境変数名が 1 つの文に
  見えていた（別の行にした）／プロフィール・セキュリティの入力欄が内容の幅までしか
  伸びていなかった／`requestId` が折り返して 1 行のログが 4 行分の高さになっていた。
- 切り欠き・ホームバー（`viewport-fit=cover` + `env(safe-area-inset-*)`）と、
  アドレスバーの伸縮（`100dvh`）に対応した。PWA として端末に入れる前提のため。
- jsdom は `<dialog>` の開閉メソッドを実装していないので、`vitest.setup.ts` が
  `open` 属性の付け外しだけを肩代わりする。

## 2026-09-02 権限を足す再投入で、消した管理者が復活しないようにした

派生アプリ（`blobshare`）の本番で、**運用の初期に削除してあった `admin@example.com`
が権限追加のマイグレーションで復活した**。パスワードはリポジトリと文書に書いてある
既定値のまま、前段に認証の無い公開サイトで。約 24 分後に無効化して削除された
（その間のログイン試行は無し）。**穴はこの雛形の側にあった。**

- **原因は `seed_master_data` が 3 つのことをしていたこと。** ロールを足す・権限を
  足す・**初期管理者が居なければ作る**。権限を足すマイグレーションは値の重複定義を
  避けるためにこの関数を呼ぶので、**権限を 1 つ足すたびに利用者の作成が走っていた。**
- ⚠ **「冪等」は運用者の決定を尊重する意味ではなかった。** 何度呼んでも同じ結果に
  なる代わりに、「この利用者は要らない」という判断を毎回上書きしていた。
- ⚠ **既定のハッシュで作るのがいちばん重い。** `ADMIN_INITIAL_PASSWORD` を渡さない
  呼び出し（＝マイグレーション）は、据え付け直後に変える前提の値を運用中の本番へ
  後から入れる。
- **`ensure_default_admin` を分けた**（ADR-0024）。据え付け（`0002`）と手で流す復旧
  （`scripts/seed_master_data.py`）だけが呼ぶ。**引数で切り替えない** ——
  書き忘れた新しいマイグレーションがどちらかに倒れるので、呼ばなければ起きない形に
  した。
- `tests/integration/test_master_data_seeder.py` が再投入で戻らないことを見張る。
- ⚠ **既に派生したアプリには自動では伝わらない。** 同じ形を当てること。

新しいものを上に追記する。細かな進捗は書かない（Progress.md 完了時に要約を移す）。

## 2026-08 ビルドとデプロイを Komodo へ一本化した

- **成果物をコンテナイメージ 1 つにした**（ADR-0023）。Komodo（nolumialab）が GitHub から
  クローンして焼き、`hub.nolumia.com:5000/komodo/<app>` へ push する。
  `latest` / `<コミット>` / `0.0.N` のタグが付き、戻すときはコミットのタグを指定する。
  - 手元でイメージの tar を作って配置先へ転送する方式（`scripts/build.sh` +
    `scripts/deploy.sh` + `scripts/build-remote-container.sh`、約 1,000 行）を撤去した。
    その大半は「配置場所から名前を決める」「コンテナ名を握った残骸を片付ける」
    「`.env` を壊さずに生成する」という、compose プロジェクトを人手で並べるがゆえの
    問題への対処で、Komodo では問題自体が無くなる。
  - GHCR へ push する `release.yml` も削除した。**そもそも通っていなかった**
    （`ruff check src/` — このリポジトリに `src/` は無い）。
  - ADR-0008 / ADR-0014 / ADR-0015 は ADR-0023 で置き換え（廃止）。
- **Komodo で焼くとイメージが版を名乗れなくなる問題を直した。** Komodo はコミット情報を
  `--build-arg` で渡さず、`.dockerignore` が `.git` を除いているので Dockerfile の中から
  git も引けない。**ビルドは緑のまま `/info` が `version=dev` を返し続ける**という、
  気付きにくい壊れ方をしていた。
  - `scripts/generate_version.sh` を Komodo Build の `pre_build` で走らせ、
    `shared/kernel/version.json` を作ってから `docker build` に入る形にした。
    build-arg は廃止（渡す側ごとに名前がずれる。`release.yml` は実際にずれていた）。
  - 優先順位は **git > 既にある version.json > dev**。Komodo はビルドディレクトリを
    使い回すため「既存を優先」にすると 2 回目以降が初回の版を名乗り続ける。逆に
    イメージ内で上書きにすると `pre_build` の結果が dev に潰れる。
  - CI（`image.yml`）で「焼いたイメージの `commit_hash` が HEAD と一致すること」を
    検証する。ここが dev に戻る変更は落ちる。
- **nginx の upstream を resolver + 変数経由にした。** `upstream { server web:8000; }` は
  起動時に 1 度だけ名前を解決してキャッシュするため、アプリのコンテナが作り直されると
  **消えた IP へ繋ぎ続けて 502 を返し続ける**。テンプレート由来の別プロジェクトが
  実際にこれで本番を落としている。設定ファイルはローカル開発とデプロイで共有する
  （`docker/nginx/default.conf.template`）。
- **サービス名を `web` → `app`、`nginx` → `front` に変えた。** compose はサービス名を
  参加する全ネットワークの別名にする。デプロイ先の `edge` はホスト全体で共有なので、
  一般名を撒くと他スタックのコンテナを掴む・掴まれる。upstream はスタック固有の別名
  （`APP_WEB_ALIAS`）で引く。
- **デプロイ定義の雛形を同梱した**（`deploy/komodo/`）。compose・Build 定義・Stack 定義を
  deploy-repo へ複製して使う。実行ユーザーの UID とデータディレクトリの所有者、
  マイグレーションと `start_period` のように**イメージと compose の両方が合っていないと
  動かない**取り決めを、イメージと一緒に配るため。
  - ついでに `db/Dockerfile` を廃止した（素の MariaDB + TZ 設定なので compose で与える）。
  - 実行ユーザー（UID 5678）とデータディレクトリの所有者が食い違っていたのも直した
    （`init-paths` が 1000 に chown していた）。

## 2026-08 押したボタンが実行中だと分かるようにした

- **サーバーへ問い合わせるボタンは、押した直後からスピナーが出て押せなくなる**
  （ADR-0022）。応答が返るまで見た目が変わらず、押せたのか分からないまま同じボタンを
  続けて押せてしまっていた。
  - 共通部品 `ActionButton` と、実行中かどうかを持つ `usePendingAction` /
    `usePendingRows` / `useLogSearch` を追加し、送信・保存・削除・切り替え・検索・
    ページ送りのすべてを揃えた。
  - 二重送信もここで止める。`disabled` は次の描画まで効かないため、同じフレームで
    続けて押された 2 つ目はフックが `ref` で判定して無視する。
  - スピナーはラベルの上に重ね、ラベルは場所を占めたまま隠す。ボタンが伸縮すると
    隣のボタンが動き、押し間違いの元になるため。読み上げ用に `common.processing` を
    視覚外へ置き、`aria-busy` も付ける。
  - 目印は**押した操作にだけ**出す。ログ画面は「どの操作が取得を起こしたか」（検索・
    クリア・失敗だけ・前へ・次へ）を保ち、初回の読み込みではどのボタンにも出さない。
  - 一覧のチェックボックス（ユーザーのロール・ロールの権限）は押しても取得し直すまで
    表示が変わらないので、その行に目印を出す。

## 2026-08 ログの保持期間を設定できるようにし、掃除を定期実行にした

- **`log` / `audit_log` に保持期間を設定できるようになった**（ADR-0021）。
  どちらも無期限に増える一方で、消す手立ては手動 SQL しかなかった
  （ADR-0013 の「今後の課題」）。`LOG_RETENTION_DAYS` と
  `AUDIT_LOG_RETENTION_DAYS` を**別々に**追加し、期限を過ぎた行を定期的に消す。
  - **既定はどちらも `0`（＝削除しない）＝従来と同じ挙動。** 保持期間は運用・監査の
    要件で決まるもので、テンプレートが勝手に消し始めてよいものではないため。
  - キーを分けたのは、アプリログを削る運用が監査記録を巻き込まないようにするため
    （ADR-0013 でテーブルを分けた理由をそのまま設定側にも通す）。
  - `0` と負の日数は同じ「削除しない」扱い。打ち間違い（`-30`）が未来の境界になって
    直近の行まで消す事故を防ぐ。境界の行は残す（`<` で比べる）。
  - 設定は掃除のたびに読み直すので、保存すれば次の周回から効く（再起動は不要）。
  - 削除した行数は `/admin/logs` に INFO で残る。何も消さなかった回は書かない。
- **定期実行の仕組みを足した**（`shared/kernel/scheduling`）。`IntervalWorker` は
  「一定間隔で 1 つの仕事を繰り返す」だけの常駐スレッドで、`RestartWatcher`
  （ADR-0004）と同じ形。スケジューラのライブラリは入れていない。
  - Gunicorn の複数ワーカーで同じ仕事が並走するため、**渡す仕事は冪等に限る**
    （排他は持たない）。「期限より古い行を消す」は冪等。
  - 起動直後に 1 回走り、以後 6 時間ごと。1 回の失敗ではスレッドを止めない。
- **削除は主キーで小分けにする。** 溜まった行を初めて消すときは数十万行が対象に
  なり得る。1 文の `DELETE` は MariaDB では長いトランザクションになり、SQLite では
  DB 全体の書き込みロックをその間握り続けてリクエストを待たせる。
  `DELETE ... LIMIT` はバックエンド依存なので「消す ID を選ぶ → その ID を消す」の
  2 段で行い、1 塊ごとにトランザクションを閉じる。

## 2026-08 本人の設定を 2 画面に整理し、狭い画面のはみ出しを直した

- **本人の設定をプロフィールとセキュリティの 2 画面にした**（ADR-0020）。
  以前は「プロフィール」「パスワード変更」「セキュリティ」の 3 画面に分かれ、
  後ろ 2 つはプロフィール下端のリンクからしか辿れなかった。パスワード変更は
  独立した画面をやめ、二要素認証・パスキーと同じ「サインインの手段」として
  セキュリティ（`/profile/security`。旧 S8 → S7）へまとめた。プロフィールには
  その入口（見出し・説明・リンク）だけを置く。旧 URL の `/change-password`・
  `/security` は `/profile/security` へ転送する。
  - 区画の中身は `components/PasswordChangeForm.tsx`・`TwoFactorControls.tsx`・
    `PasskeyControls.tsx` に分け、区画の見出しと枠は画面側が持つ
    （`PreferenceControls` と同じ形）。CSS クラスは 2 画面で共有するため
    `.profile-section` / `.profile-form` → `.settings-section` / `.settings-form`。
  - 画面が 1 つ減り、画面一覧の採番が繰り上がった（旧 S9〜S15 → S8〜S14）。
- **ロールの切り替えメニューが画面の外へ開いていたのを直した。** 狭い画面では
  ヘッダーが折り返して切り替えボタンが行の左端へ回り込むことがあり、そこから
  右寄せ（`right: 0`）で開くメニューが画面の左外へはみ出して選べなかった。
  狭い画面では位置の基準をヘッダー（`position: sticky`）へ移し、ヘッダーの幅
  いっぱいに開くようにした。併せて、収まらないロール名は末尾を省略し、
  折り返してもヘッダーの操作は右寄せのままにした。
- **システムステータスの時刻がカードからはみ出していたのを直した。**
  `確認時刻（UTC）` の ISO 形式（`2026-08-01T06:41:34.624372+00:00`）は
  分割できず、値の列（`1fr`）を押し広げていた。列を `minmax(0, 1fr)` にし、
  狭い画面では名前の下に値を積んで 1 行に収まるようにした。

## 2026-08 アプリログの強化 — 失敗を残し、死活監視を残さない

- **失敗が記録されるようになった**（ADR-0019）。`HTTPException`・入力検証エラー・
  ドメイン例外はルーターを抜けた先で応答へ変わるため、これまではアクセスログの
  1 行（ステータスコードだけ）しか残らなかった。記録を
  `presentation/fastapi/error_handling.py` に集約し、`request_failed`（エラーコード
  付き）・`request_validation_failed`（落ちた項目名と理由。**入力値は入れない**）を
  残すようにした。
- **想定外の例外の受け皿を入れた。** `Exception` ハンドラが未登録で、500 は
  Starlette の `ServerErrorMiddleware`（全ミドルウェアの外側）が処理していた。
  traceback は DB に残らず、アクセスログにも 500 の行が出ず、ログのまとめ書きも
  飛ばされてそのリクエストのログ行ごと消えていた。`InternalErrorMiddleware`
  （最も内側）で応答へ変え、`{"detail": {"error": "internal_error"}}` の JSON と
  `X-Request-Id` を返すようにした（別オリジンからも本文を読める）。
- **ログのレベルをステータスコードで決めるようにした。** 5xx → ERROR、4xx →
  WARNING、401 と成功 → INFO。WARNING 以上で絞れば失敗だけが残る。
- **死活監視のパスをアクセスログから外した。** `/healthz` `/readyz` `/api/health`
  `/metrics` の成功は残さない（Docker の healthcheck が数十秒おきに叩き、`log` の
  大半を占めていた）。**失敗（4xx/5xx）は残す。**
- **ログ基盤自身の失敗が見えるようになった。** `DbLogHandler` は書き込み失敗を
  `handleError()`（stderr）で知らせる（従来は `pass`）。設定の DB 読み取りが
  できない状態も、状態が変わったときに 1 行だけ警告する（`system_settings_unreadable`）。

## 2026-08 ロールの切り替え（複数ロールの使い分け）と画面デザインの刷新

- **アクティブロールを導入**（ADR-0017）。ユーザーは以前から複数ロールを持てたが、
  有効 scope は常に全ロールの和集合で、使う権限を選べなかった。JWT に
  `active_role` クレームを足し、`POST /api/auth/switch-role` によるトークンの
  再発行で切り替えるようにした。
  - `null`（既定）= 従来どおり全ロールの和集合。ロールを 1 つ選ぶとその分だけ。
  - 保有していないロールへは切り替えられない（403 `role_not_granted`）。権限は
    減る方向にしか動かない。
  - refresh トークンにもクレームを載せ、更新しても選択が続く。切り替えた後に
    そのロールを外されると、そのトークンの scope は空になる。
  - 切り替えは監査ログへ `role.switched` として残る。
- **`GET /api/auth/me` が `roles` と `active_role` を返す**ようになった。`scopes` は
  「保有権限」ではなく「**いま有効な権限**」の意味になっている。
- **ヘッダーにロールの切り替えを追加**（複数ロールを持つ利用者にだけ出る）。
  プロフィールにはロールの一覧（操作中のものに印）を足した。
- **ユーザー管理で複数ロールを扱えるようにした**。追加フォームは複数選択、一覧は
  ロールごとの列のチェックで付け外しする（従来は追加時に 1 つ選ぶだけだった）。
- **ロール一覧の取得を `user:manage` でも通すようにした**（ADR-0018）。ユーザーへ
  ロールを割り当てる画面が「どんなロールがあるか」を知る必要があるため。作成・
  更新・削除は `role:manage` のまま。「いずれか 1 つ」を宣言する
  `require_any_permission()` を追加した（読み取り専用）。
- **画面デザインを刷新**。配色・角丸・影・間隔を CSS 変数へ集約し、カードを浮かせた
  レイアウト、ぼかし付きの固定ヘッダー、選択中の項目を縦線とアクセント色で示す
  サイドバー、格子を描かない表、ボタンの種類（`.button-primary` /
  `.button-danger` / `.button-ghost`）、チップ表示（`.chip` / `.chip-list`）を
  入れた。ライト・ダークの両テーマと狭い画面の扱いは従来どおり。

## 2026-08 プロフィールの自己編集・メニュー階層化・システムステータス画面

- **プロフィールページを拡充**（ADR-0016）。メールアドレス・表示名を本人が
  `PUT /api/auth/me` で変更できるようにし（重複メールは `email_already_exists`）、
  変更は監査ログへ `profile.updated`（変更した項目名のみ）として残す。
  言語・テーマの切り替えはヘッダー／ドロワー下端からプロフィールの「表示設定」へ
  移した（保存先はブラウザのまま。ADR-0005 は不変）。
- **サイドバーを階層化**。ユーザー・ロール・権限は「管理」、システム設定・
  システムログ・監査ログ・システムステータスは「システム管理」のグループにまとめた。
  表示できる項目が無いグループは見出しごと出さない。
- **システムステータス画面を追加**（`/admin/system-status`、要 `system:manage`）。
  `GET /api/admin/system/status` がビルド情報（バージョン・git SHA・ブランチ・
  ビルド日時・環境）と API・DB の状態、稼働時間を返す。

## 2026-07 デプロイの名前を配置場所から決め、テンプレート名のままでは動かさない

テンプレートから作ったプロジェクトが元テンプレートと同じ名前（`fastapitemplate`）を
名乗り続けていた。イメージタグ・compose プロジェクト名・DB コンテナ名・ネットワーク名が
すべてこの名前から導かれるため、テンプレート由来の別プロジェクトを同じホストで動かすと、
同じ `container_name` とホストポートを奪い合う。過去 2 回の deploy 修正
（ネットワークの重複 / 他プロジェクトのコンテナの切り離し。ADR-0014）はこの衝突の症状だった。

改名だけでは「衝突しうる構造」が残る（次に名前が当たればまた同じことが起きる）ため、
`deploy.sh` の**置き場所からデプロイの名前を決める**方式にした（ADR-0015）。

```
/volume1/docker/rewardpointsweb/prod/deploy.sh   → アプリ名 rewardpointsweb・環境 prod
/volume1/docker/kaimono/stg/deploy.sh            → アプリ名 kaimono・環境 stg
```

アプリ名は親ディレクトリ名、環境は自分のディレクトリ名から取る。別のアプリは別の
ディレクトリに置かれる以上、名前は構造的に衝突しない。環境名を `basename` で決めていたのと
同じ考え方をアプリ名へ広げた形。

- 優先順位は `.env` の `APP_NAME` ＞ 親ディレクトリ名 ＞ `BUILD_APP_NAME`。
- ディレクトリ名は docker の識別子として使うため小文字英数と `-` `_` へ正規化する
  （`RewardPoints Web` → `rewardpoints-web`）。
- 実際に使われた名前と出所は起動時に 1 行で出力する。
- 解決した名前がテンプレート名のままならデプロイを中断する（テンプレートである以上、
  書き換え忘れは必ず起こるため、警告ではなく中断にした）。
- `BUILD_APP_NAME` は「デプロイの名前」ではなく「`image.tar` の中身がどう tag されて
  いるか」で、`manifest.env` が無いときの load 後の参照先にだけ使う。
- `DB_CONTAINER_NAME` / `DOCKER_NETWORK_NAME` も解決した値を compose へ `export` する
  ようにした。`docker-compose.yml` 側の既定値だけが効くと、衝突を調べるときに見る名前と
  compose が実際に作る名前がずれるため。

旧名からの移行として、旧アプリ名の compose プロジェクトを一度だけ畳み、自動生成した
`.env` の旧既定名を書き換える。畳む対象は compose の
`com.docker.compose.project.working_dir` ラベルがこの環境ディレクトリと一致するコンテナ
だけで構成されているときに限る（プロジェクト名ラベルの絞り込みは docker デーモン全体を
見るため、それだけを根拠にすると同じ名前を使う別のアプリを停止・削除してしまう）。
永続データはホスト側の `HOST_DATA_ROOT` にあるため消えない。

移行手順はディレクトリごとの改名なので、旧コンテナの `working_dir` ラベルと自動生成された
`.env` の `HOST_DATA_ROOT`（絶対パス）は**移動前のパスを指したまま**残る。前者を見落とすと
旧デプロイを畳めずホストポートを握られたままになり、後者を見落とすと消えたパスを
`mkdir -p` が作り直して**空の DB で MariaDB が初期化される**。どちらも移動前のパス
ちょうどと一致し、かつそこが実在しないときだけ自分のものとして扱う（まだディレクトリが
あるなら別のデプロイが現役でいるということなので触れない）。

## 2026-07 パスワード欄に表示切り替え（目のアイコン）を追加

伏せ字のままでは打ち間違いに気付けず、サインインや変更が失敗しても原因が入力ミスか
どうか分からなかった。共通コンポーネント `frontend/src/components/PasswordInput.tsx`
を追加し、入力欄の右の目のアイコンで平文表示へ切り替えられるようにした。

- 対象はサインイン・パスワード再設定・パスワード変更（現在／新しい）・ユーザー追加の
  各パスワード欄と、システム設定の秘匿項目（`MAIL_PASSWORD` 等）。
- 既定は伏せ字。表示状態は欄ごとに独立し、保存しない（再描画で元に戻る）。
- 切り替えボタンには `aria-label`（表示中／非表示中で切り替わる）と `aria-pressed` を
  付け、入力欄が `disabled` のときはボタンも押せない。
- 枠線は既存の `input` / `button` と同じ `var(--border)` をそのまま使い、
  欄ごとに枠色がずれないようにした（`index.css` の `.password-field` /
  `.password-toggle`）。

併せて、パスワード欄の見出しを `<label>` で囲む形から `for` / `id` で結ぶ形に変えた。
`<label>` の中に入れられる labelable な要素は対応付けの相手 1 つだけで、入力欄と
表示切り替えボタンの 2 つが入ると支援技術から見た対応付けが曖昧になるため
（HTML の `label` の内容モデル）。囲めない欄は `.field` で従来と同じ縦積みにする。

この変更で、システム設定（`/admin/config`）の行が **見出しと入力欄の縦積みから、
見出し左・入力欄右の横並びに変わった**。従来は行そのものが `<label>` だったため
`.card label`（縦積み）が `.config-row`（`justify-content: space-between`）より
詳細度で勝っており、意図された横並びにならなかった。行を `<div>` にしたことで
`.config-row` の指定がそのまま効くようになった（狭い画面で縦積みにする
メディアクエリは従来どおり）。

## 2026-07 コンテナ名の衝突を「消せた」と誤認してデプロイが落ちる不具合の修正

`./deploy.sh reset`（prod）が、名前の衝突を自動で解消する仕組み（ADR-0014）を通った
うえで落ちた。ログ上は残骸を消してやり直しているのに、やり直した `up` が
**まったく同じコンテナ ID** との衝突で失敗し、診断の `docker compose ps -a` も
名前の持ち主一覧も空、という追えない出力になっていた。

**原因**: `docker rm -f` は**コンテナが存在しないときも成功（終了コード 0）で返す**
（Docker 20.10 以降）。実体が無いまま名前だけ Docker が握っている残骸
（ADR-0014 の経路 2）はまさにこれに当たるため、終了コードだけを見ていた
`remove_leftover_container` が「消せた」と誤認していた。結果、ADR-0014 が用意していた
「デーモンの再起動を促して止まる」分岐へ入れず、同じ衝突でもう一度落ちたうえ、
コンテナが 1 つも無いのでモジュールログも空になっていた。

**修正**:

- `docker rm -f` の終了コードではなく**出力**で実体の有無を見分けるようにした
  （`No such container` なら名前は空いていない）。この場合は再試行せず、
  デーモンの再起動が必要である旨を出して止まる（ADR-0014 の決定どおり）。
- デーモンの再起動を促すのは「実体が無い」と分かったときだけにした。削除がそれ以外の
  理由（認可プラグインによる拒否・ストレージやデーモンの一時的なエラー等）で失敗した
  ときにまで再起動を促すと、実際の原因を取り違えさせるため。この場合は捕まえた
  メッセージをそのまま見せ、通常どおりモジュールログ付きの診断を出す。
- やり直した `up` が**前回と同じ相手**と衝突したときも同じ診断を出すようにした。
  解放されていない名前を握ったままの再試行は何度やっても同じ結果になるため。
- 実体の無い名前が原因のときは、空になると分かっているモジュールログを出さない。
- 名前の持ち主一覧が 1 件も無いときに `(none)` と明示するようにした。「持ち主が
  見当たらないのに名前は使用中」がこの状態の決め手であり、空行では伝わらないため。
- 持ち主を引く `docker ps -a --filter` のパターンを `^/?<名前>$` にした。先頭の `/`
  の有無は Docker のバージョンで変わり、`^/<名前>$` では引けない環境があった。

## 2026-07 デプロイがコンテナ名の衝突で失敗する不具合の修正

`./deploy.sh reset`（prod）が `docker compose down` の成功直後に

```
Error response from daemon: Conflict. The container name
"/fastapitemplate-init-paths-1" is already in use by container "7754001e8a0e…".
```

で落ちた。診断に出る `docker compose ps -a` は空で、ログからは原因が追えなかった。

**原因**: `docker compose down` が消すのはこのプロジェクトのラベル
（`com.docker.compose.project`）を持つコンテナだけで、同じ名前を握った残骸までは
面倒を見ない。残骸は `docker compose ps` にも映らないため、「down は全部消えた・
ps も空・なのに名前が使用中」という状態になっていた。残骸の出どころは 2 通りある。
compose 管理外で作られた（ラベルを持たない）コンテナと、削除処理が途中で止まり
実体は無いのに名前だけ Docker が掴んでいる状態。

**修正**（ADR-0014）:

- `down` に `--remove-orphans` を付けた。過去の compose ファイルにしか無いサービスの
  コンテナが残って名前を取り合うのを防ぐ。
- `up` の前に、このデプロイが作るコンテナ名（`<プロジェクト>-<サービス>-1` と
  `.env` の `DB_CONTAINER_NAME`）を握っている残骸を消すようにした。
- 名前だけ掴まれている残骸は `inspect` にも `ps` にも出ないため先回りできない。
  `up` の失敗メッセージに入っている衝突相手のコンテナ ID を取り出して消し、
  1 度だけやり直すようにした。
- **消すのは残骸だけ。** 相手が別の compose プロジェクトのコンテナなら、稼働中の
  stg を黙って落としかねないので消さずに中断し、名前の重複を直すよう促す。
- 診断に「このデプロイが使うコンテナ名を今誰が握っているか」を追加した
  （`docker compose ps` はプロジェクト外の持ち主を映さないため）。
- `up` 失敗時に同じ出力を 2 度出していた（`tee` の後にもう一度 `cat` していた）のをやめた。

## 2026-07 監査ログの追加と、ログ画面の検索・絞り込み

DB のログを「システムが何をしたか」（`log`）と「誰が何をしたか」（`audit_log`）に
分け、どちらも管理画面から検索できるようにした（ADR-0013、idp の設計を参考にした）。

これまで答えられなかった問い合わせが 3 つあった。「このユーザーを無効にしたのは誰か」
（実行者と対象が記録されていない）、「ログイン失敗が続くアカウントはあるか」
（失敗は例外で終わり何も残らない）、「先週の ERROR をこのモジュールだけ見たい」
（絞り込みがレベルと `requestId` だけ）。

- **`audit_log` テーブル（新規）**: `bounded_contexts/audit/` として記録・閲覧を
  持つ。記録するのはログイン成否・パスワード変更／リセット・ユーザーとロールの
  作成／更新／削除・システム設定変更・再起動要求・二要素認証とパスキーの有効化／解除。
  実行者・対象は内部 ID のみで、PII（メールアドレス・パスワード・設定値）は記録しない。
  `reason` には失敗の分類（`invalid_password`）や変更した項目名（`fields=is_active`）が入る。
- **失敗も必ず残る**: 監査イベントは処理の途中では控えに積むだけにし、リクエストの
  処理が完全に終わってから別トランザクションでまとめて書く。ログイン失敗は 401 で
  リクエストがロールバックされるため同じセッションでは記録が消え、かといって処理の
  途中で別コネクションから書くと SQLite の書き込みロックと衝突する（既定の
  `sqlite:///app.db` で `database is locked` になり、記録だけ失われる）。
- **実行者は認証済みのときだけ記録する**: ログイン失敗・パスワードリセットは未認証で
  叩けるため、相手のアカウントは実行者ではなく**対象**として記録する。持ち主を実行者に
  据えると、第三者が起こしたイベントが本人の操作として残ってしまう。
- **設定キー `TRUSTED_PROXY_HOPS`（新規、既定 `0`）**: `X-Forwarded-For` は送信元が
  自由に付けられるため、既定では見ずに TCP の接続元を使う。同梱の nginx 構成
  （docker-compose）では `1` を設定し、プロキシが自分で足した値を採る。
- **絞り込みの追加**: アプリログはレベル・ロガーの前方一致・本文の部分一致・
  `requestId`・期間で、監査ログはイベント・結果・実行者・対象・`requestId`・期間で
  絞れる。どちらも総件数付きのページ送りに対応した。前方一致・部分一致の入力に含まれる
  `%` `_` はエスケープする（パターンとして解釈しない）。
- **新しい scope `audit:view`**: 監査ログの閲覧は `log:view` では許可しない。
  画面 `/admin/audit-logs` を追加（S14）。
- **設定キー `LOG_DB_MIN_LEVEL`（新規、既定 `INFO`）**: DB へ書く下限レベルを stdout と
  独立に上げられる。保存と同時に反映される（再起動不要）。
- **`log` に索引を追加**: `level` / `logger` / `user_id_hash`（新しい絞り込み軸）。
- **破壊的変更**: `GET /api/admin/logs` の応答が配列から
  `{"total": n, "entries": [...]}` になった。ページ送りの可否を画面が判断できないため。
- `PasswordResetService.request_reset` / `reset` が対象利用者の ID を返すようになった
  （監査ログに「誰の」要求かを残すため。API の応答は従来と同じ）。

設計品質の定量基準（ADR-0012）に合わせて 3 か所を組み直した。

- **`LoginService`（新規）**: ログインの資格情報検証・第二要素・失敗の理由分けと、
  それぞれの監査記録をルーターから切り出した。ルーターは受け取った利用者をトークンと
  Cookie に載せるだけになり、依存も 5 個から 3 個へ減った。
- **検索条件の正規化を Presentation へ寄せた**: 空文字を「未指定」に丸めるのは
  `〇〇Request`（Pydantic）の役割にし、Domain の検索条件は正規化済みの値だけを受ける
  （CLAUDE.md「API 設計」の「Application 層へはバリデーション済みの値のみを渡す」に沿う）。
- **実行者の差し替えを `RecordAuditEvent.as_actor()` に限定した**: 呼び出しごとに
  実行者を渡せると、未認証の操作にうっかり相手を実行者として渡せてしまう。実行者が
  処理の最中に確定するのはログインだけなので、そこだけ束ね直す。

## 2026-07 設計品質の定量基準を機械検証するようにした

CLAUDE.md が挙げていた定量基準（ネスト深度 3・関数長 30 行・引数 3 個・複雑度 10・
クラス長 200 行）はレビュー時の観点にとどまり、Backend では検証されていなかった。
まず既存コードの違反数を計測し（関数 407 個・クラス 128 個）、**基準どおりの閾値で
導入して既存の違反を直す**方針を採った（ADR-0012）。ほとんどの基準が違反 0 だったため。

- **Ruff**: `C901`(10) / `PLR0912`(8) / `PLR0913`(4) / `PLR0915`(30) を有効化。
- **AST テスト**（`tests/unit/test_design_metrics.py` 新規）: Ruff にルールが無い
  ネスト深度 3 とクラス長 200 行を検査する（依存方向テストと同じ方式）。
- 基準に対してずらしたのは 2 点だけ。**引数は 4 で検証**する（Ruff は `Depends()` や
  キーワード専用引数も数えるため）。**関数長は行数ではなく文の数**で見る（日本語の
  docstring や折り返した型注釈を数えると、説明を削る方向に働くため）。
- 基準に合わせて 3 か所を修正した。ログ一覧 API のクエリパラメータを
  `LogSearchRequest` へまとめ（OpenAPI 上のパラメータは不変）、
  **どこからも渡されていなかった `RestartRequestStore.save()` の `requested_at` 引数を削除**、
  依存方向テストの 5 段ネストを 2 段へ。
- `make check` と CI のコマンドは変わらない（`ruff check` と `pytest` に相乗り）。

## 2026-07 画面をスマートフォン対応にした

`index.css` にメディアクエリが 1 つも無く、390px 幅ではサイドバーが本文を圧迫し、
表がページ全体を横スクロールさせていた。ブレークポイント 1 つ（768px）を入れ、
狭い画面ではサイドバーを開閉式のドロワーにした（ADR-0011）。

- **ナビゲーション**: ヘッダーの ☰ で開閉。閉じるのは ✕ ・画面外タップ・Esc・
  項目の選択の 4 通り。開閉状態を持つのは新設の `components/AppLayout.tsx` で、
  **コンポーネントは画面幅を判定しない**（幅の解釈は CSS に閉じる）。
- **言語・テーマの選択**を `components/PreferenceControls.tsx` へ切り出し、狭い画面では
  ヘッダーではなくドロワーの下端に出す（ヘッダーが 3 段に折り返すのを避けるため）。
- **表**を `.table-scroll` で包み、表だけが横スクロールするようにした（6 画面）。
  ページ全体は横スクロールしない（390px で検証）。
- 入力欄・ボタンの最小高さを 2.75rem、文字を 16px にした（iOS Safari の自動拡大を防ぐ）。
  チェックボックスは枠ではなく目盛りを 1.5rem に拡大する。
- 横並びのフォーム（`.inline-form`）と設定行（`.config-row`）は縦に積む。
- **キーボード操作**: 開いたらドロワー内へフォーカスを移し、Tab をドロワー内で折り返し、
  閉じたらメニューボタンへ戻す（覆われて見えないヘッダーへ Tab で到達できてしまうため）。
  あわせて `visibility` の遷移時間を 0 にした。`visibility` は離散的に変わるため
  `transition: visibility 0.2s` と書くと切り替わりが遅れ、**開いた直後はまだ
  `hidden` = フォーカス不可**で `focus()` が黙って無視されていた（jsdom は CSS を
  適用しないためテストでは再現せず、実ブラウザでのみ起きる）。

## 2026-07 PWA アイコンを差し替えた（maskable を別画像に分けた）

アイコンを「白い F の文字」から稲妻マーク（対角グラデーションの角丸）に差し替えた。
あわせて**マニフェストの `maskable` が通常アイコンと同じ画像を指していた不具合を直した**。
Android はホーム画面のアイコンを円などに切り抜くため、セーフゾーンの無い画像では
ロゴの端が切れていた。`pwa-maskable-512x512.png`（全面塗り・図形を中央 42% に縮小）を
別に用意して割り当てた。iOS 用の `apple-touch-icon.png` は OS 側が角丸に切るため、
透明な角を持たない全面塗りにした。

配色と図形の正本は `scripts/generate_pwa_icons.py`（標準ライブラリだけで PNG を書き出す
生成スクリプト）で、`favicon.svg` を含む 5 ファイルすべてをそこから生成する。
手作業でアイコンを差し替えると SVG と PNG がずれるため、変更手順は `OPERATIONS.md` に置いた。

## 2026-07 DB のポートをホストへ公開しなくした

`db` は保守用に `127.0.0.1:3307:3306` をホストへ公開していたが、アプリは Docker
ネットワーク内部の `db:3306` へ接続するため、この公開は保守作業のためだけにあった。
`ports` を外して `expose: ["3306"]` だけにし、保守は `docker compose exec db mariadb ...`
とコンテナ内で完結させるようにした（ADR-0010）。ホストへ公開されるのは nginx の 1 ポートだけ。

- 設定キー `DB_BIND_ADDR` / `DB_HOST_PORT` を廃止した。`deploy.sh` が生成する `.env` にも
  `DB_HOST_PORT` を書かなくなった（stg / prod の分離はコンテナ名とネットワーク名で足りる）。
  **既存の `.env` に残っている行は compose が参照しないため無視される**（`deploy.sh` は
  既存の `.env` を書き換えないので、そのままデプロイできる）。次のデプロイでホスト側の
  3307 / 3308 が解放される。
- GUI クライアントから一時的につなぐ手順（socat のポートフォワード用コンテナ）を
  `OPERATIONS.md` に追加した。恒久的に開ける設定は持たない。
- **`docker compose exec` の資格情報は `sh -c '…'` でコンテナ内に展開させる。**
  `.env` は既定では資格情報の行がコメントアウトされており（値は compose の
  `${VAR:-default}` が供給する）、ホスト側のシェルに変数が無い。ダブルクォートで
  書くと空文字に展開されて接続に失敗する。既存の「管理者パスワード復旧」手順にも
  同じ誤りがあったので直した。

## 2026-07 CI の重複実行をなくした

`push`（全ブランチ）と `pull_request` の両方がトリガーだったため、**PR のあるブランチへ
push すると同じコミットが 2 回検証されていた**（Backend / Frontend × 2 イベント =
チェック 4 個）。内容は完全に同一で、CI 時間がそのまま二重になっていた。

`push` の対象を `main` だけに絞り、フィーチャーブランチは `pull_request` 側だけで
検証するようにした（ADR-0009）。あわせて `concurrency` を設定し、PR への連続 push では
古い実行をキャンセルする（`main` は「どのコミットが緑だったか」の記録を残すためキャンセルしない）。

**PR を作っていないブランチへの push では CI が走らなくなった。** PR を作れば全ゲートが走る。
効いていなかった `tags-ignore: ["v*"]` も削除した（`branches` を指定したワークフローは
タグ push に一致しないため元々無効。`v*` タグは `release.yml` の担当）。

## 2026-07 WEB 公開ポートの既定値を build-remote-container.env で指定できるようにした

デプロイ先の WEB 公開ポートは `deploy.sh` の環境別既定値（stg=8081 / prod=8080）か、初回
デプロイで生成された `.env` を手で直すしかなかった。8080 が空いていないホストでは
「一度失敗させてから `.env` を直して再実行」する手順になっていたため、
`build-remote-container.env` に `APP_WEB_HOST_PORT` を書けるようにした（ADR-0008）。

- `build-remote-container.sh` が `APP_WEB_HOST_PORT` を DEPLOY ステップへ引き継ぎ、
  `deploy.sh` が `.env` を新規生成するときの `WEB_HOST_PORT` に転記する。
  優先順位は `.env` ＞ `APP_WEB_HOST_PORT` ＞ 環境ディレクトリ名由来の既定値。
  既存の `.env` は従来どおり書き換えない。
- 値の検証（1〜65535 の整数）でデプロイを中断するのは、その値が実際に選ばれたときだけ。
  `.env` に `WEB_HOST_PORT` がある環境では使われないため、古い値・打ち間違いが残っていても
  デプロイは止めず警告で知らせる。先頭 0 付きの値は 8 進数と解釈される前に弾く。
- **`deploy.sh` が解決後の `WEB_HOST_PORT` を `export` するようにした（不具合修正）。**
  従来は compose へ渡っておらず、`.env` に `WEB_HOST_PORT` の行が無い環境では
  compose 側の既定値 8080 だけが効き、stg のヘルスチェック（8081）とずれていた。
  公開ポートとヘルスチェック URL が常に同じ値を見るようになった。

## 2026-07 画面仕様・ER 図の追加とドキュメント更新義務の明文化

「現在の仕様」を示すドキュメントが 2 種類欠けていたため追加し、更新義務を
CLAUDE.md に書いた（ADR-0007）。

- **`frontend/README.md`（新規）**: 画面遷移図（Mermaid）・画面一覧（ルートと必要
  scope）・画面ごとの仕様・利用者向けの操作マニュアル。これまでルートと画面の
  必要 scope は `App.tsx` と `Sidebar.tsx` を読まないと分からなかった。
- **`docs/ER.md`（新規）**: ER 図（Mermaid）・テーブル定義・モデリング規約。
  12 テーブルの関係、`log` が `users` へ FK を張らない理由（PII を持たない方針）、
  `account_security` の 3 テーブルが `ON DELETE CASCADE` である点を図に含めた。
- **CLAUDE.md**: 「変更したら必ず更新するドキュメント」の表を追加。画面を変えたら
  `frontend/README.md`、スキーマを変えたら `docs/ER.md` を**同じコミットで**更新する。
  あわせて `Progress.md` の運用（着手前に行を追加し、完了時に削除。「✅完了」は作らない）
  と ADR の基準（対象・採番規則・承認済みは書き換えず置き換える）を明文化した。

**初期管理者のパスワードを `admin` から `admin@example.com` に変更した。**
平文は `master_data.DEFAULT_ADMIN_PASSWORD` に定数として持たせ、事前計算ハッシュ
（`DEFAULT_ADMIN_PASSWORD_HASH`）との対応を `tests/unit/test_master_data.py` が検証する。
Domain 層は werkzeug を import できないため、ハッシュ生成をここに置けないため。
テストからパスワード文字列の直書きも無くした。

## 2026-07 品質ゲートの必須化（整形・静的解析・型・テスト）

CI を「Lint → Type Check → Test」の必須ゲートにした（ADR-0006）。
Backend 4 種・Frontend 4 種の計 8 ゲートで、落ちたらマージできない。
経緯は `history/2026-07-quality-gates.md`。

導入したツールと、それによって見つかった実際の不具合:

- **Ruff Format**（新規）。162 ファイル中 100 ファイルが未整形だった。一括整形した。
  `line-length` を 100 → 120 に上げ、`E501` の一律除外をやめた。
- **Ruff Check の拡張**。`select` に `C4` / `ARG` / `N` / `RET` / `PTH` / `RUF` を追加。
  `DomainException` → `DomainError`（N818）、`int(round(...))` の二重変換（RUF046）、
  未使用引数、`__all__` の未ソート、効かない `# noqa` 14 件を整理した。
- **MyPy strict**（新規）。本番コードとテストの両方を対象にし、114 件を修正。
  - `RestartWatcher` が具象 `RestartRequestStore` に依存していた **DIP 違反**を検出。
    `RestartRequestReader` Protocol を切り出し、watcher はそれだけに依存するようにした。
    テストダブルを渡すのに具象クラスの継承が要らなくなった。
  - `sessionmaker` / `dict` / `Callable` の型引数漏れ、`Result` に無い `rowcount` への
    アクセス、`FromClause.insert()`（`sa.insert(Log)` へ修正）を検出。
  - `require_permission()` が戻り値の型を持っていなかった（依存関数ファクトリ）。
- **TypeScript の追加フラグ**。`noUncheckedIndexedAccess` /
  `exactOptionalPropertyTypes` / `noImplicitOverride` /
  `noFallthroughCasesInSwitch` を有効化。空配列を前提にした `locales[0]`、
  `choices` を `string[][]` と緩く持っていた箇所（`[value, label]` のタプルへ）、
  `fetch` に `body: undefined` を渡していた箇所を直した。
- **ESLint**（新規）。`strictTypeChecked` + `react-hooks` + `sonarjs` で 74 件。
  最多は **`no-misused-promises` 19 件**で、`onSubmit={submit}` のように async 関数を
  そのままハンドラへ渡し、Promise を投げ捨てていた箇所。内部で `catch` 済みなので
  `void` で明示に変えた。`main.tsx` の非 null 断言、`response.json()` の `any`、
  `String(unknown)` による `[object Object]` の混入も直した。
- **Vitest**（新規）。Frontend にテストが 1 本も無かったため 27 件を追加
  （i18n の言語選択・プレースホルダ、テーマの OS 追従と購読解除、API クライアントの
  トークン保持とエラーコード変換、UI 設定取得のフォールバック）。
- **DDD 依存方向の検証**（新規）。`tests/unit/test_layer_dependencies.py` が AST で
  「Domain は Application / Infrastructure / Presentation を import しない」
  「Application は Infrastructure / Presentation へ依存しない」
  「Domain は FastAPI / SQLAlchemy 等に依存しない」を検証する（93 ケース）。

`make check` で CI と同じものを手元で流せる（`make format` で自動整形）。

## 2026-07 二要素認証・パスキー・テーマ切り替え・自己再起動

photonest を参考に 5 つの機能を追加し、重複していた処理を整理した。

- **二要素認証（TOTP）とパスキー（WebAuthn）** を
  `bounded_contexts/account_security/` として追加（ADR-0003）。
  移植元にあった 2 つの問題を設計で直した。
  - 共有鍵を `users` の列ではなく `totp_secrets` テーブルに置き、確認できるまで
    有効にしない 2 段階登録にした（QR の読み取り失敗で締め出されないように）。
  - WebAuthn チャレンジをプロセス内 `dict` ではなく `webauthn_challenges` テーブルに
    置いた。移植元の実装は単一プロセス専用で、既定の Gunicorn `--workers=2` では
    発行と検証が別ワーカーに当たった瞬間に必ず失敗する状態だった。
  - `pyotp` / `webauthn` は Infrastructure に閉じ込め、Domain には Protocol だけを置いた。
- **設定変更による自己再起動** を `shared/kernel/restart/` として追加（ADR-0004）。
  起動時にしか読まれない設定（`LOG_LEVEL` / `LOG_TO_DATABASE` /
  `CORS_ALLOWED_ORIGINS`）は保存しても反映されず、画面上は成功と出ていた。
  保存 API が `restart_required` を返し、`POST /api/admin/system/restart` で
  再起動を要求できるようにした。
- **テーマ切り替え**（light / dark / OS 追従）を追加。配色を CSS 変数へ移し、
  `<html data-theme>` で切り替える。以前はブラウザ任せの `Canvas` /
  `CanvasText` を使っており、アプリ側から配色を選べなかった。
- **日英切り替えの仕上げ**。`LANGUAGES` / `DEFAULT_LOCALE` は定義済みだったが
  参照するコードが無く、管理画面に並ぶだけで何も動かしていなかった。
  公開エンドポイント `GET /api/ui/settings` で配り、実際に効くようにした
  （ADR-0005）。管理画面の設定ラベル・選択肢も辞書で訳せるようにし、
  訳の抜けを `tests/unit/test_i18n_dictionaries.py` が検出する。

重複処理の整理:

- `POST /api/admin/maintenance/shutdown` を削除。自プロセスへ SIGTERM を送るだけで、
  Gunicorn 配下ではワーカーが 1 つ落ちてアービターが同じ環境で作り直すため、設定は
  反映されなかった。終了方法の判断を `build_process_terminator()` に集約し、
  `POST /api/admin/system/restart` へ一本化した。
- `system_settings` テーブルを短命コネクションで読む生 SQL が設定解決と再起動要求で
  重複していたため、`SystemSettingRecordReader` に集約した。
- アクセストークン Cookie の付与を `set_access_token_cookie()` に集約
  （パスワード・リフレッシュ・パスキーの 3 経路が同じ属性を使う）。
- `utcnow()` を `shared/kernel/timestamps.py` へ移動（Application 層からも使うため）。
- フロントエンドの 7 か所に散っていた「例外 → `error.<code>` 翻訳キー」変換を
  `errorMessageKey()` に集約した。

## 2026-07 ビルド／デプロイ最新化・PWA 対応

- ビルドを `scripts/build.sh` に集約（idp と同方式）。`dist/` に image tar・`deploy.sh`・
  `manifest.env`／`manifest.sha256`（checksum・イメージ ID 照合）を出力する。
- git 非搭載のデプロイ先向けに `scripts/build-remote-container.sh` を導入
  （dev コンテナ内で SYNC → BUILD → PICK → DEPLOY を一括実行。self-update 対応）。
- `deploy.sh` の配置を dist 直下（`<env>/deploy.sh`）に統一（旧 `<env>/scripts/deploy.sh`
  配置は廃止）。manifest による tar 検証とロード済みイメージの再利用を追加。
- DB のホスト公開ポートを既定でループバック（127.0.0.1）に限定（`DB_BIND_ADDR`）。
  公開ポートは nginx のみ。
- フロントエンドを PWA 化（vite-plugin-pwa: Web App Manifest・Service Worker 自動更新・
  アイコン一式。`/api` 等はナビゲーションフォールバック対象外）。

## 2026-07 テンプレート刷新（photonest 準拠）

- photonest の構成・設計思想をベースに全面刷新。
  DDD 4層 + bounded_contexts 構成、scope ベース認可（JWT）、
  システム設定管理（環境変数 > DB > デフォルト）、構造化ログ（JSON + DB）、
  React SPA スケルトン、Docker（db / web / nginx）、デプロイスクリプトを導入。
- アルバム・メディア・バッチ（Celery / Redis）・wiki・Google 連携は持ち込まない。
- 設計判断は ADR-0001（DB エンジン）・ADR-0002（認証スコープ）を参照。
