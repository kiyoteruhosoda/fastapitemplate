...
移行手順はディレクトリごとの改名なので、旧コンテナの `working_dir` ラベルと自動生成された
`.env` の `HOST_DATA_ROOT`（絶対パス）は**移動前のパスを指したまま**残る。前者を見落とすと
旧デプロイを畳めずホストポートを握られたままになり、後者を見落とすと消えたパスを
`mkdir -p` が作り直して**空の DB で MariaDB が初期化される**。どちらも移動前のパス
ちょうどと一致し、かつそこが実在しないときだけ自分のものとして扱う（まだディレクトリが
あるなら別のデプロイが現役でいるということなので触れない）。
