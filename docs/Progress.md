...
| 5 | T12 | 「止めたら届く」の残り 2 つ（短命のアクセストークン・定期照合） | ⬜未着手 | 大 | 中 |
| 6 | T14 | ローカルのログインにも多要素を強制できるようにする | ⬜未着手 | 大 | 中 |
| 7 | T13 | 本人の操作で IdP の口座と結び付ける導線（設定画面から連携） | ⬜未着手 | 中 | 中 |
| 8 | T6 | 一覧 API のページング規約を決めて見本を置く | ⬜未着手 | 中 | 中 |
| 9 | T7 | 公開面の露出を絞る（`/metrics`・`/info`・nginx のヘッダー） | ⬜未着手 | 中 | 小 |
| 10 | T8 | 依存ライブラリの脆弱性検査を CI に足す | ⬜未着手 | 中 | 小 |
| 11 | T9 | 期限切れ・使用済みパスワードリセットトークンを掃除する | ⬜未着手 | 中 | 小 |
| 12 | T10 | `example` コンテキストを「作って終わり」から本当の見本へ育てる | ⬜未着手 | 中 | 中 |
| 13 | T11 | テンプレート名を付け替えるスクリプト | ⬜未着手 | 中 | 中 |
...
アクセストークン**・**定期照合**。1 つ目は ADR-0036 で入った。残りは 2 つ。

- **アクセストークンの寿命**が、受け口の落ちていたときの上限時間になる。既定 15 分が
  妥当かを決める（決めたら `.env.example` の注記も直す）。
...
### T13 本人の操作で IdP の口座と結び付ける導線

ADR-0037 で `OIDC_LINK_BY_EMAIL` の既定を倒したので、**同じメールアドレスの
ローカル口座を持つ人が初めて SSO で入ると断られる**（`sso_account_not_linked`）。
いまの逃げ道は「設定を開ける」か「管理者が `federated_identities` を整える」の 2 つ
しか無い。
...
- 解除も要る（解除したあと入れなくなる人を作らないこと。ローカルの資格情報が
  1 つも無いなら断る）
