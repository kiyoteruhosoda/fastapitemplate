...
| 優先 | # | 概要 | 状態 | 影響度 | 工数 |
|---|---|---|---|---|---|
| 1 | T16 | 入力面を整理してスマホでの画面の使い方を直す（追加フォームのダイアログ化・絞り込みの折りたたみ・表と一覧の密度） | 🚧進行中 | 中 | 中 |
...
| 13 | T13 | 外部 IdP との SSO（`identity_federation` の逆輸入） | ⬜未着手 | 大 | 大 |
| 14 | T14 | ローカルの入口を閉じられるようにする（`LOCAL_LOGIN_ENABLED`） | ⬜未着手 | 大 | 中 |
| 15 | T15 | 機械の口（`client_credentials` のトークンで API を叩く経路） | 🟡要判断 | 中 | 大 |

## 詳細

### T16 入力面を整理してスマホでの画面の使い方を直す

390px 幅で開くと、どの管理画面も**最初の 1 画面が入力欄で埋まり、その画面が扱う
データ（一覧）は下へ押し出されている**。ユーザー管理は追加フォーム 5 項目、監査ログは
絞り込み 8 項目で、どちらもスクロールしないと 1 件も見えない。

- 追加フォーム（S5・S8・S9）を「追加」ボタン + ダイアログへ移す。狭い画面では
  画面下から**幅いっぱい**で出る（ADR-0027）。
- ログの絞り込み（S12・S13）は狭い画面で畳み、「絞り込み」で開く。
- 権限・ロールの対応表は、行の見出しを固定し、列見出しを縦組みにして
  幅あたりの情報量を上げる（`th.vertical` が指定されていながら定義が無かった）。
- システムステータス・システム設定の名前と値の並びが縦に間延びしている点も詰める。
