...
| 優先 | # | 概要 | 状態 | 影響度 | 工数 |
|---|---|---|---|---|---|
| 1 | T3 | PWA アイコンを差し替える（maskable を別図形にする） | 🚧進行中 | 小 | 小 |
| 3 | T4 | 画面をスマホ対応にする（レスポンシブ・ナビゲーション） | 🚧進行中 | 中 | 中 |
| 4 | T1 | Backend の定量的な設計品質基準（関数長・引数数・複雑度）を機械検証する | 🚧進行中 | 中 | 中 |

## 詳細

### T3 PWA アイコンを差し替える

`pwa-192x192.png` / `pwa-512x512.png` / `apple-touch-icon.png` / `favicon.svg` を
差し替える。現状の maskable アイコンは通常アイコンと同じ画像を指しており、
Android のマスク（円形など）でロゴが切れる。セーフゾーンを取った
maskable 専用画像を別に用意する。
