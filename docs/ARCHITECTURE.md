...
監査イベントは処理の途中では**控えに積むだけ**で（I/O なし・失敗しない）、リクエストの
処理が完全に終わってから**別トランザクションでまとめて書く**（audit コンテキストの
ミドルウェア）。理由は 2 つ。

- 失敗したログインは 401 で終わり、リクエストのセッションはロールバックされる。
  同じセッションで書くと最も記録したいイベントが消える。
- 処理の途中で別コネクションから書くと、SQLite ではリクエストのセッションが握った
  書き込みロックと衝突する（`database is locked`）。

書き込みは**素の ASGI ミドルウェア**で行う。FastAPI は `yield` を使う依存
（`get_db` の commit）をレスポンス送出の後に閉じるため、`BaseHTTPMiddleware` の
`call_next` が戻った時点ではまだセッションが開いている。記録の失敗では本処理を
落とさない（アプリログへ ERROR で残して続行する）。

実行者（`actor_user_id`）を入れるのは**認証済みのリクエストだけ**。未認証で叩ける
操作（ログイン失敗・パスワードリセット）は「誰がやったか」が分かっていないので、
分かっている「誰に対しての操作か」を `target` に入れる。
...
**失敗の記録は `presentation/fastapi/error_handling.py` に集約する**（ADR-0019）。
...
| `HTTPException`・ドメイン例外 | `request_failed`（+ `error_code`） | `presentation.fastapi.error_handling` |
| 入力検証エラー | `request_validation_failed`（+ `invalid_fields`。値は入れない） | 同上 |
...
- 想定外の例外は `InternalErrorMiddleware`（最も内側）が応答へ変える。ここで
  受け止めないと、アクセスログの 500 の行も、ログのまとめ書きも飛ばされる。
