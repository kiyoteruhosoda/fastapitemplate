# frontend — 画面仕様・操作マニュアル

管理画面 SPA（React + TypeScript + Vite）の**画面遷移図・画面仕様・操作マニュアル**。

この文書は「画面が**現在どう動くか**」の仕様書である（役割分担は
[CLAUDE.md](../CLAUDE.md)「ドキュメントの役割分担」参照）。過去の不具合の経緯は
`docs/CHANGELOG.md`、設計判断は `docs/decisions/`（ADR）に書く。

> **更新ルール**: 画面の追加・削除、ルート（URL）の変更、遷移の変更、必要 scope の
> 変更、画面上の操作手順の変更を行ったときは、**同じコミットでこのファイルを更新する**。
> 新しい画面を追加するときは「画面遷移図」「画面一覧」「画面仕様」「操作マニュアル」の
> 4 か所すべてに追記する。

API のリクエスト・レスポンス仕様は手書きしない。Swagger UI（`/docs`）と
`/openapi.json` が唯一の出所で、本書は「どの画面がどのエンドポイントを使うか」だけを示す。

---

## 開発

```bash
npm install
npm run dev          # http://localhost:5173（/api は 8000 へプロキシ）
npm run build        # frontend/dist → FastAPI が / で配信
npm run test         # Vitest
```

パスキーを試すときは **`localhost`** で開く（`127.0.0.1` では WebAuthn の RP ID が
成立しない）。

---

## セッションの持ち方

**この SPA はトークンを持たない**（ADR-0028）。access / refresh とも httpOnly Cookie で
運ばれ、JavaScript からは見えない。そのため:

- ログイン済みかどうかは **`GET /api/auth/me` の成否**で判断する（手元の値を見ない）。
- 更新系（POST / PUT / PATCH / DELETE）には **CSRF の二重送信トークン**
  （`csrf_token` Cookie の値を `X-CSRF-Token` ヘッダーへ）を載せる。
  `src/services/api.ts` が毎回 Cookie から読み直す ——セッションを張り直すたびに
  新しくなるので、控えると古い値を送ることになる。
- ログアウトで Cookie を落とすのはサーバー側（httpOnly なのでこちらからは消せない）。
  画面はまず手元の控えを捨ててから `POST /api/auth/logout` の完了を待つ
  （待ってから捨てると、回線が遅いときに押しても何も起きないように見える）。
- **サインアウトを IdP まで通すかはサーバーが決める**（`OIDC_RP_LOGOUT_ENABLED`。
  既定は無効。ADR-0033）。判断は **`/api/auth/me` の `rp_logout_enabled`** に載って
  届く（サインアウトのために問い合わせを増やさないため）。真のときだけ、アプリの
  サインアウトを終えたあと `GET /api/auth/sso/logout` へ**画面遷移**する
  （IdP の Cookie を落とすのは IdP 自身なので fetch では届かない）。
  戻りは `GET /api/auth/sso/signed-out` 経由で `/login?signed_out=1`。

## 画面遷移図

```mermaid
flowchart TD
    subgraph public["未認証（RequireAuth の外）"]
        Login["/login<br/>サインイン"]
        Totp["/login（TOTP 入力ステップ）<br/>※同一画面内の状態遷移"]
        Forgot["/forgot-password<br/>リセットリンク送信"]
        Reset["/reset-password?token=…<br/>新しいパスワード設定"]
        SsoCallback["/login/sso?ticket=…<br/>SSO の戻り（引き換え）"]
        Idp[["外部 IdP<br/>（別サイト）"]]
    end

    subgraph private["認証済み（Header + Sidebar + Footer レイアウト）"]
        Dashboard["/<br/>ダッシュボード"]
        Items["/items<br/>アイテム"]
        Profile["/profile<br/>プロフィール"]
        Security["/profile/security<br/>セキュリティ<br/>（パスワード・2FA・パスキー・連携）"]
        Users["/admin/users<br/>ユーザー管理"]
        Roles["/admin/roles<br/>ロール管理"]
        Perms["/admin/permissions<br/>権限一覧"]
        Config["/admin/config<br/>システム設定"]
        Logs["/admin/logs<br/>システムログ"]
        Audit["/admin/audit-logs<br/>監査ログ"]
        Status["/admin/system-status<br/>システムステータス"]
    end

    Login -->|"パスワード認証成功"| Dashboard
    Login -->|"totp_required"| Totp
    Totp -->|"コード検証成功"| Dashboard
    Totp -->|"戻る"| Login
    Login -->|"パスキーでサインイン"| Dashboard
    Login -->|"パスワードをお忘れですか"| Forgot
    Forgot -->|"メール内のリンク"| Reset
    Reset -->|"更新完了"| Login

    private -.->|"未認証・トークン失効"| Login
    Dashboard -->|"ログアウト"| Login

    Dashboard --> Items
    Dashboard --> Users
    Dashboard --> Roles
    Dashboard --> Perms
    Dashboard --> Config
    Dashboard --> Logs
    Dashboard --> Audit
    Dashboard --> Status
    Logs <-->|"同じ requestId で突き合わせる"| Audit
    Dashboard -->|"ヘッダーのユーザー名"| Profile
    Profile <-->|"「セキュリティ設定を開く」／「プロフィールへ戻る」"| Security
```

- サイドバーの項目は**ロール名ではなく scope** で出し入れする（`components/Sidebar.tsx`）。
  メニューは階層化されており、ユーザー・ロール・権限は「管理」、システム設定・
  システムログ・監査ログ・システムステータスは「システム管理」のグループに入る
  （表示できる項目が 1 つも無いグループは見出しごと出ない）。
- 未認証で認証必須ルートを開くと `/login` にリダイレクトされる（`RequireAuth`）。
- 未定義のパス（`*`）は `/` にリダイレクトされる。
- 旧 URL の `/change-password`・`/security` は `/profile/security` へ転送される（ADR-0020）。

---

※ SSO を有効にしている場合、`/login` の「SSO でログイン」から外部 IdP へ出て、
`/login/sso?ticket=…` へ戻る（ADR-0025）。`Login --> Idp --> SsoCallback --> Dashboard`。
往復に失敗すると `/login?sso_error=<コード>` へ戻る。

※ `rp_logout_enabled` が真のとき、ログアウトは `/login` で終わらず IdP へ出て
`/login?signed_out=1` へ戻る（ADR-0033）。`Dashboard --> Idp --> Login`。

## 画面一覧

| #   | 画面                   | ルート                    | 認証 | 必要 scope                                          | サイドバー |
| --- | ---------------------- | ------------------------- | ---- | --------------------------------------------------- | ---------- |
| S1  | サインイン             | `/login`                  | 不要 | —                                                   | —          |
| S2  | パスワードリセット申請 | `/forgot-password`        | 不要 | —                                                   | —          |
| S3  | パスワード再設定       | `/reset-password?token=…` | 不要 | —                                                   | —          |
| S15 | SSO の戻り             | `/login/sso?ticket=…`     | 不要 | —                                                   | —          |
| S4  | ダッシュボード         | `/`                       | 必要 | `dashboard:view`                                    | ✅         |
| S5  | アイテム               | `/items`                  | 必要 | `item:view`（追加は `item:manage`）                 | ✅         |
| S6  | プロフィール           | `/profile`                | 必要 | —                                                   | —          |
| S7  | セキュリティ           | `/profile/security`       | 必要 | —                                                   | —          |
| S8  | ユーザー管理           | `/admin/users`            | 必要 | `user:manage`                                       | ✅         |
| S9  | ロール管理             | `/admin/roles`            | 必要 | `role:manage`（一覧の取得のみ `user:manage` も可）  | ✅         |
| S10 | 権限一覧               | `/admin/permissions`      | 必要 | `permission:manage`                                 | ✅         |
| S11 | システム設定           | `/admin/config`           | 必要 | `admin:system-settings`（再起動は `system:manage`） | ✅         |
| S12 | システムログ           | `/admin/logs`             | 必要 | `log:view`                                          | ✅         |
| S13 | 監査ログ               | `/admin/audit-logs`       | 必要 | `audit:view`                                        | ✅         |
| S14 | システムステータス     | `/admin/system-status`    | 必要 | `system:manage`                                     | ✅         |

サイドバーの階層: S4・S5 は最上位、S8〜S10 は「管理」、S11〜S14 は「システム管理」の
グループに入る。S6・S7 はサイドバーに出さない（scope ではなく本人かどうかで決まる
画面のため。ADR-0020）。

scope の一覧と各ロールへの割り当ての正本は `shared/domain/auth/master_data.py`。

---

## 画面仕様

各画面は「目的 / 表示内容 / 操作 / 使用 API / 備考」で書く。新しい画面もこの形に揃える。

**追加（作成）は「追加」ボタン + ダイアログで行う**（S5・S8・S9）。画面には一覧が
先に出て、フォームは押したときだけ開く。狭い画面では画面の下から**幅いっぱい**の
シートとして開き、広い画面では中央に開く。閉じるのは ✕ ・「キャンセル」・Esc ・
背景のタップのいずれか（`src/components/FormDialog.tsx`。ADR-0027）。

パスワードの入力欄には共通で目のアイコンが付く（`src/components/PasswordInput.tsx`）。
押すと入力内容を平文で表示し、もう一度押すと伏せ字へ戻る。既定は伏せ字で、表示状態は
欄ごとに独立し、画面を離れると元に戻る（保存しない）。対象は S1・S3・S7・S8 の
パスワード欄と、S11 の秘匿項目（`MAIL_PASSWORD` 等）。

### S1 サインイン（`/login`）

- **目的**: メールアドレスとパスワード、パスキー、または外部 IdP（SSO）でサインインする。
- **表示内容**: SSO ボタン（有効なときだけ。名前は `OIDC_DISPLAY_NAME`）、メール、
  パスワード、サインインボタン、パスキーボタン（ブラウザが対応している場合のみ）、
  パスワードリセットへのリンク。
  - **どの入口を出すかはサーバーに聞く**（`GET /api/auth/sso/provider`）。
    `local_login_enabled` が偽ならパスワード欄とパスキーのボタンを出さない
    （ADR-0026 決定 2）。**問い合わせに失敗したときは出したままにする**——
    ここで隠すと、問い合わせが落ちただけで全員が締め出される。
  - `?sso_error=<コード>` が付いていれば、`error.<コード>` の文言で失敗を表示する。
  - `?signed_out=1` は IdP でのサインアウトを終えて戻ってきた印（ADR-0033）。
    `login.signedOut` を出す。**出さないと、期限切れで飛ばされたのと見分けが
    付かない。**送信すると畳む（済んだ話なので残さない）。
- **操作**:
  1. メール・パスワードを入力して送信 → 成功で `/` へ。
  2. 二要素認証が有効なユーザーは `totp_required` が返り、**同じ画面が
     ワンタイムコード入力ステップに切り替わる**（エラー表示ではなく次の手順）。
  3. コードを送信 → 成功で `/`。「戻る」で資格情報入力に戻る。
- **使用 API**: `POST /api/auth/login`, `POST /api/auth/passkey/challenge`,
  `POST /api/auth/passkey/login`, `GET /api/auth/sso/provider`
  （SSO ボタンは `GET /api/auth/sso/login` へ**画面遷移**する。fetch では IdP の
  ログイン画面を出せないため）
- **備考**: エラーはエラーコードで返り、文言は `src/i18n/*.json` の
  `error.*` キーで表示する（バックエンドは表示文言を返さない）。

### S15 SSO の戻り（`/login/sso?ticket=…`）

- **目的**: 外部 IdP からの戻りで受け取った引き換え券をトークンへ換える（ADR-0025）。
- **表示内容**: 「ログインしています…」だけ。利用者の操作は無い。
- **操作**: 自動。成功したら SSO を始めた画面へ、失敗したら
  `/login?sso_error=<コード>` へ転送する。
- **使用 API**: `POST /api/auth/sso/token`
- **備考**: **URL に載るのは券だけでトークンは載らない**（URL は履歴・Referer・
  プロキシのログに残るため）。券は 1 回限りで、React の開発時の二重実行でも
  2 度換えにいかないよう画面側で控えている。

### S2 パスワードリセット申請（`/forgot-password`）

- **目的**: 登録済みメールアドレスへ再設定リンクを送る。
- **操作**: メールを入力して送信 → 宛先の存在に関わらず同じ完了メッセージを表示する
  （アカウントの存在を漏らさないため）。
- **使用 API**: `POST /api/auth/forgot-password`

### S3 パスワード再設定（`/reset-password?token=…`）

- **目的**: メール内リンクのトークンで新しいパスワードを設定する。
- **操作**: 新しいパスワードを入力して送信 → 完了後 `/login` へ。
- **使用 API**: `POST /api/auth/reset-password`
- **備考**: トークンは失効・使用済みで拒否される（`error.invalid_or_expired_token`）。

### S4 ダッシュボード（`/`）

- **目的**: サインイン後の入口。
- **表示内容**: 見出しのみのスケルトン。プロジェクト固有のウィジェットを足す場所。
- **使用 API**: なし

### S5 アイテム（`/items`）

- **目的**: `bounded_contexts/example`（Item CRUD）の画面側の見本。
- **操作**: 一覧の表示と、「アイテムを追加」から開くダイアログでの追加。
  追加ボタンは `item:manage` を持つ人にだけ出る。
- **使用 API**: `GET /api/items`, `POST /api/items`

### S6 プロフィール（`/profile`）

- **目的**: サインイン中のユーザー自身のアカウント情報の確認・変更と、表示設定
  （言語・テーマ）の変更（ADR-0016）。
- **表示内容**: 「アカウント」（メールアドレス・ユーザー名の編集フォーム）、
  「セキュリティ」（S7 への入口。見出しと説明と「セキュリティ設定を開く」
  ボタンのみ）、「表示設定」
  （言語・テーマのセレクタ）、「ロール」（割り当てられているロール。操作中のものに
  「使用中」が付く）、「有効なスコープ」（**いま**使える scope。ロールを絞っている
  あいだはその範囲だけ）。
- **操作**:
  1. メールアドレス・ユーザー名を編集して「保存」。重複するメールアドレスは
     `error.email_already_exists` で拒否される。
  2. 言語・テーマを切り替える（即時反映。選択はブラウザに保存される）。
  3. 「セキュリティ設定を開く」で S7 へ。
- **使用 API**: `GET /api/auth/me`, `PUT /api/auth/me`
- **備考**: メールアドレスを変えても再サインインは不要（トークンはユーザー ID で
  引き直す）。次回のサインインから新しいメールアドレスを使う。変更は監査ログに
  `profile.updated` として残る（変更した項目名のみ。値は記録しない）。

### S7 セキュリティ（`/profile/security`）

- **目的**: サインインの手段（パスワード・二要素認証・パスキー・IdP との連携）を
  まとめて扱う（ADR-0020 / ADR-0040）。
- **表示内容**: 「パスワード変更」（現在のパスワード・新しいパスワード）、
  「二要素認証」（有効・無効の状態、登録時の QR コードと手入力用の鍵）、
  「パスキー」（追加フォームと登録済みの一覧: 名前・登録日時・最終使用日時）、
  「シングルサインオン」（連携の有無と連携日時、連携・解除のボタン）、
  プロフィール（S6）へ戻るリンク。
- **操作**:
  1. 現在のパスワードと新しいパスワードを入力して「変更」。
  2. 二要素認証: 「設定する」→ QR を認証アプリで読み取り → コードを入力して確定。
     解除は現在のコードの入力が必要。
  3. パスキー: 名前を入れて「パスキーを追加」→ ブラウザの認証を実行 → 登録。
     一覧から削除。
  4. シングルサインオン: 「〇〇 と連携する」→ IdP の画面で認証 → この画面へ戻る。
     解除は「連携を解除する」。
- **使用 API**: `POST /api/auth/change-password`,
  `GET|POST /api/account/security/two-factor*`,
  `GET|POST|DELETE /api/account/security/passkeys*`,
  `GET|DELETE /api/auth/sso/link`, `POST /api/auth/sso/link/start`
- **備考**:
  - 入口はプロフィール（S6）の「セキュリティ」だけ（サイドバーには出さない）。
  - 旧 URL の `/change-password`・`/security` はこの画面へ転送する。
  - ブラウザが WebAuthn 非対応のときは「パスキー」に非対応メッセージを出す。
  - ⚠ **パスワードを持たない利用者（`/api/auth/me` の `has_password` が偽）には、
    変更フォームの代わりに案内文を出す**（ADR-0038）。SSO でしか入れない利用者は
    「現在のパスワード」を入力できないので、フォームを出すと絶対に通らない入力欄に
    なる。
  - **SSO が無効な環境では「シングルサインオン」の区画そのものを出さない。**
  - ⚠ **IdP へ遷移するのは画面自身である。** サーバーは認可要求の URL を JSON で
    返すだけ（ADR-0040 決定 2）。認証が切れていた場合に 401 を受け取って、
    やり直しの導線を出せるようにするため。
  - ⚠ **外すと入れなくなる利用者には、解除のボタンを出さずに理由を出す**
    （ADR-0040 決定 5）。数えるのはパスワードとパスキーで、二要素認証は数えない。
  - IdP から戻った直後は URL に `?sso_link=linked` か `?sso_link_error=<コード>` が
    付く。読み終えたら画面が消す（残すと再読み込みのたびに同じ報せが出る）。

### S8 ユーザー管理（`/admin/users`）

- **目的**: ユーザーの追加・ロールの付け外し・有効/無効の切り替え・削除。
- **表示内容**: 一覧（ユーザー名・ID・メールアドレス・**ロールごとに 1 列**の
  チェックボックス・有効フラグ・**入れる手段**・削除）と、見出しの横の「ユーザーを追加」。
  ロールの列は列見出しを縦組みにし、ユーザー名の列は横スクロールしても左端に残る
  （対応表の共通の扱い。ADR-0027）。
- **操作**:
  1. 「ユーザーを追加」でダイアログを開き、メール・ユーザー名・パスワードを入れ、
     必要なロールを**いくつでも**選んで「追加」。
  2. 一覧のロール列のチェックで付け外しする（即時反映）。ロールを外すと、その人の
     権限はそのぶん減る。
  3. 一時的に止めるときは「有効」を切り替える。
- **使用 API**: `GET|POST /api/admin/users`, `PUT|DELETE /api/admin/users/{id}`,
  `GET /api/admin/roles`（`user:manage` でも読める。ADR-0018）
- **備考**:
  - 複数ロールを持つ利用者は、**どのロールで操作するか**を自分でヘッダーから
    切り替える（ADR-0017）。管理者が選ぶのは「何を持たせるか」だけ。
  - ロール一覧を取得できない場合でも、既に割り当てられているロールは列に残る
    （割り当ての表示を失わないため）。
  - 更新中の行は操作を受け付けない（続けて押すと、後の更新が前の更新を
    打ち消してしまうため）。
  - **「入れる手段」はこのアプリが知っている口の棚卸し**（ADR-0039）——パスワード /
    二要素認証 / パスキーの本数 / SSO。⚠ **ここからは取り上げられない**（表示だけ。
    取り上げ方はそれぞれの口の側にある）。⚠ **IdP 側の多要素は出ない** ——出せるのは
    このアプリが知っている口だけで、それがまさに「認証系が 2 つある」ということである。

### S9 ロール管理（`/admin/roles`）

- **目的**: ロールの追加・削除と、ロールへの権限（scope）割り当て。
- **表示内容**: ロール × 権限の対応表（権限コードごとに 1 列のチェックボックス）と、
  見出しの横の「ロールを追加」。権限コードは長いので列見出しを縦組みにし、
  ロール名の列は横スクロールしても左端に残る（ADR-0027）。
- **操作**:
  1. 「ロールを追加」でダイアログを開き、ロール名を入れて「追加」。
     **権限を持たない状態で作られる**ので、続けて表のチェックで割り当てる。
  2. 表のチェックで権限を付け外しする（即時反映）。
- **使用 API**: `GET|POST /api/admin/roles`, `PUT|DELETE /api/admin/roles/{id}`,
  `GET /api/admin/permissions`

### S10 権限一覧（`/admin/permissions`）

- **目的**: 定義済み権限コード（scope）の確認。読み取り専用。
- **使用 API**: `GET /api/admin/permissions`

### S11 システム設定（`/admin/config`）

- **目的**: DB に保存する設定値をカテゴリ別に編集する。
- **表示内容**: カテゴリごとの設定項目。環境変数で固定されている項目は
  「環境変数で固定」と表示して編集不可、起動時のみ読まれる項目は
  「再起動後に反映」と表示する。
- **操作**: 値を編集して保存 → 再起動が必要な項目を含む場合は対象キーを示す
  メッセージと「今すぐ再起動」ボタンを出す。`system:manage` が無い場合は
  管理者への依頼メッセージを表示する。
- **使用 API**: `GET|PUT /api/admin/config`, `POST /api/admin/system/restart`
- **備考**: 項目の定義は `presentation/fastapi/admin/system_settings_definitions.py`。
  設定キーを追加するときは CLAUDE.md「設定管理」の 3 ファイルを更新する。

### S12 システムログ（`/admin/logs`）

- **目的**: `log` テーブル（アプリログ = **システムが何をしたか**）の検索・閲覧。
- **表示内容**: 時刻（UTC）・レベル・ロガー・メッセージ・`requestId`・パス・
  ステータス・所要時間。件数は「{総件数} 件中 {開始}〜{終了} 件」で示す。
  例外が記録された行は「traceback を表示」で本文の下に展開する。
- **操作**:
  1. 絞り込み条件（レベル／ロガーの前方一致／メッセージの部分一致／`requestId`／
     期間）を入れて「検索」。未入力の項目は条件に使われない。
     **狭い画面では絞り込みは畳まれている**ので、「絞り込み」を押して開く（ADR-0027）。
  2. 「条件をクリア」で全条件を消す。
  3. 「前へ」「次へ」でページを送る（1 ページ 50 件、新しい順）。
- **使用 API**: `GET /api/admin/logs`, `GET /api/admin/logs/filters`
- **備考**:
  - ログに PII は含まれない。ユーザーの識別は `user.id_hash` のみ。
  - 期間の指定と時刻の表示はどちらも **UTC**（ブラウザのローカル時刻ではない）。
  - レベルの選択肢はバックエンドの `GET /api/admin/logs/filters` から取得する
    （画面に列挙を写さない）。
  - DB に書かれる下限レベルはシステム設定の `LOG_DB_MIN_LEVEL`。ここに出ないログは
    stdout 側には出ている可能性がある。
  - 狭い画面では絞り込みが 1 列に積まれ、表だけが横スクロールする（列が多いため）。

### S13 監査ログ（`/admin/audit-logs`）

- **目的**: `audit_log` テーブル（監査ログ = **誰が何をしたか**）の検索・閲覧。
  ログイン成否・ユーザー／ロール管理・ロールの切り替え・システム設定変更・
  再起動要求・二要素認証とパスキーの変更が記録されている。
- **表示内容**: 時刻（UTC）・イベント・結果（success / failure）・実行者（ユーザー ID）・
  対象（`種別:ID`）・詳細・IP アドレス・`requestId`。
- **操作**:
  1. 絞り込み条件（イベント／結果／実行者のユーザー ID／対象の種別・ID／`requestId`／
     期間）を入れて「検索」。**狭い画面では絞り込みは畳まれている**ので、
     「絞り込み」を押して開く（ADR-0027）。
  2. 「失敗だけ表示」で `result=failure` に一発で絞る（他の条件はクリアされる）。
  3. 「条件をクリア」で全条件を消す。「前へ」「次へ」でページを送る。
- **使用 API**: `GET /api/admin/audit-logs`, `GET /api/admin/audit-logs/filters`
- **備考**:
  - 実行者・対象は**内部 ID** で表示する。監査ログにメールアドレス等の PII を保存しない
    ため（`docs/decisions/ADR-0013-audit-log.md`）。ID から利用者を辿るときは
    ユーザー管理（S8）を見る。
  - 「詳細」列には失敗の分類（`invalid_password` 等）や変更した項目名
    （`fields=is_active`・`keys=LOG_LEVEL`）が入る。**値そのものは入らない**。
  - **「実行者」が入るのは認証済みの操作だけ。** ログイン失敗やパスワードリセットは
    誰が行ったか分からないため、相手のアカウントは「対象」列に出る（実行者は `—`）。
    あるアカウントに対する不審な操作を追うときは「対象の種別」＝`user` で絞る。
  - ログイン失敗の応答は理由を問わず同じ（`invalid_credentials`）。理由の内訳を
    見られるのはこの画面だけ。
  - ログアウトは記録されない（操作した利用者を特定できないため）。
  - 狭い画面での扱いは S12 と同じ（絞り込みは畳まれ、表だけが横スクロールする）。

### S14 システムステータス（`/admin/system-status`）

- **目的**: 稼働中インスタンスのバージョンと各コンポーネント（API・DB）の状態確認。
- **表示内容**: 「コンポーネント」（API・データベースの 正常/異常、稼働時間）、
  「ビルド情報」（バージョン・git SHA・ブランチ・ビルド日時・環境・確認時刻）。
- **操作**: 「再読み込み」で最新の状態を取り直す。
- **使用 API**: `GET /api/admin/system/status`
- **備考**: DB の状態は `/readyz` と同じ `SELECT 1` で確認する。認証不要の稼働監視には
  この画面ではなく `/healthz`・`/readyz` を使う。

### 共通レイアウト（Header / Sidebar / Footer）

- **Header**: アプリ名（`/` へ戻る）、**ロールの切り替え**、ユーザー名（頭文字の
  アバター付き。`/profile` へ）、ログアウト。狭い画面ではメニューボタン（☰）が
  左端に出て、ユーザー名は隠れてアバターだけになる。言語・テーマの切り替えは
  プロフィール（S6）にある。
  - ログアウトは手元の状態をその場で捨て、`POST /api/auth/logout` の完了を待つ。
    サーバーが `rp_logout_enabled` を真で返す構成では、そのあと IdP へ
    画面遷移する（ADR-0033）。既定では遷移しない。
  - ロールの切り替え（`components/RoleSwitcher.tsx`。ADR-0017）は
    **複数ロールを持つ利用者にだけ**出る。押すと「すべてのロール」と保有ロールが
    並び、選ぶとその場でトークンが再発行されて有効 scope が変わる
    （サイドバーの項目もそれに追従する）。閉じるのは Esc かメニューの外側。
- **Sidebar**: S4・S5 と S8〜S14 のうち、**保有 scope に合致する項目だけ**を表示する。
  階層は「画面一覧」の節のとおり（管理／システム管理のグループ）。
- **Footer**: バージョンと git SHA（`GET /info`）。
- 言語・テーマの初期値はサインイン前に `GET /api/ui/settings` から取得する。

#### 実行中の表示（全画面共通）

サーバーへ問い合わせるボタン（送信・保存・削除・切り替え・検索・ページ送り）は、
押した直後からボタンの中にスピナーが出て、終わるまで押せなくなる（ADR-0022）。
**押したボタンにだけ**出る（並んだボタンのどれが動いているか分かるようにするため）。
ボタンの大きさは変わらないので、隣のボタンの位置もずれない。終わらないうちに
続けて押しても 2 度目は送られない。

一覧の中のチェックボックス（ユーザーのロール、ロールの権限）は、押しても結果を
取得し直すまで表示が変わらないため、その行の操作欄にスピナーが出る。

#### 新しい版の知らせ（全画面共通）

新しい版が焼かれて配られると、画面の下に **「新しい版があります。[再読み込み]」**
が出る（ADR-0035）。押すまで消えず、押したときだけ新しい版に入れ替わる。
押さないあいだは古い版のまま動き続ける（SPA は画面を切り替えても読み込み直さない）。

出るまでの遅れは最大 30 分。タブを手前に戻したときにも確認するので、
別のアプリから戻ってきたときに出ることが多い。

#### 画面幅による違い（768px で切り替わる）

|                    | 広い画面（769px 以上）     | 狭い画面（768px 以下）                                              |
| ------------------ | -------------------------- | ------------------------------------------------------------------- |
| サイドバー         | 本文の左に常に表示         | ☰ で開くドロワー（閉じるのは ✕ / 画面外タップ / Esc / 項目を選ぶ） |
| 表                 | そのまま表示               | 表だけが横スクロールする（画面全体は横スクロールしない）            |
| 横並びのフォーム   | 横に並ぶ                   | 縦に積み、入力欄は画面幅いっぱい                                    |
| 追加のダイアログ   | 中央に開く（最大 30rem）   | 画面の下から**幅いっぱい**のシートとして開く。決定と取り消しは等分  |
| ログの絞り込み     | 常に開いた状態（畳めない） | 既定で畳み、「絞り込み」で開く                                      |
| 見出しと追加ボタン | 横に並ぶ（ボタンは右端）   | 縦に積み、ボタンは幅いっぱい                                        |
| ロールの切り替え   | ボタンの真下に開く         | ヘッダーの幅いっぱいに開く（ロール名は収まらなければ省略する）      |
| 名前と値の一覧     | 名前と値を横に並べる       | 名前の下に値を積む（ISO 形式の時刻が途中で折り返らないように）      |
| 一覧の中のコード   | 折り返して全体を見せる     | 折り返さない（1 行のログが数行分の高さにならないように）            |
| 設定名と環境変数名 | 横に並べる                 | 環境変数名を下の行へ落とす                                          |

ドロワーの開閉状態は `components/AppLayout.tsx`、絞り込みの開閉は
`components/FilterPanel.tsx` が持ち、それぞれ `data-open` 属性に書き込む。
**どの幅で何が変わるかは CSS（`src/index.css` のメディアクエリ）が唯一の出所**で、
コンポーネント側は幅を判定しない（ADR-0011）。

切り欠き・ホームバーのある端末では、ヘッダー・本文・フッター・シートの余白が
`env(safe-area-inset-*)` の分だけ広がる（`index.html` の `viewport-fit=cover`）。
画面の高さは `100dvh`（アドレスバーの伸縮に追従する）。

---

## 操作マニュアル

「〇〇したいとき、どう操作するか」を利用者視点で書く。画面の操作を変えたら
ここも直す。

### サインインする

1. `/login` を開く。
2. メールアドレスとパスワードを入力して「サインイン」。
   - 初期管理者は `admin@example.com` / `admin@example.com`（初回サインイン後に必ず変更する）。
3. 二要素認証を有効にしている場合は、認証アプリのコードを入力して再度「サインイン」。
4. パスキーを登録済みなら「パスキーでサインイン」から画面ロック / セキュリティキーで入れる。
5. SSO を有効にしている場合は「SSO でログイン」から外部 IdP へ出て、認証が済むと戻ってくる。

> パスワード欄が出ていないときは、ローカルの入口が閉じられている（`LOCAL_LOGIN_ENABLED`）。
> SSO のボタンからログインする。

### パスワードを忘れたとき

1. `/login` の「パスワードをお忘れですか」を開く。
2. メールアドレスを入力して送信する。
3. 届いたメールのリンク（`/reset-password?token=…`）を開き、新しいパスワードを設定する。
4. `/login` に戻ってサインインする。

> メール送信が無効（`MAIL_ENABLED` が off）の環境ではリンクは届かない。その場合は
> `user:manage` を持つ管理者にパスワードの上書きを依頼する（画面には項目が無いため
> API から行う）。手順は `docs/OPERATIONS.md`「管理者がパスワードを忘れて
> サインインできないとき」を参照。投入スクリプトの再実行では復旧しない
> （既存ユーザーを変更しないため）。

### パスワードを変更したいとき

1. ヘッダーのユーザー名 → 「セキュリティ設定を開く」（S7）。
2. 「パスワード変更」に現在のパスワードと新しいパスワードを入力して「変更」。

> SSO でしか入れない利用者には、入力欄の代わりに案内文が出る（ADR-0038）。
> パスワードで入れるようにするには、管理者に設定してもらう。

### パスワードの打ち間違いを確かめたいとき

1. パスワードの入力欄の右にある目のアイコンを押す。入力内容が平文で表示される。
2. もう一度押すと伏せ字に戻る。

> 人に見られる場所では確認したらすぐ戻す。表示したままでも送信内容は変わらない。

### 二要素認証を有効にしたいとき

1. ヘッダーのユーザー名 → 「セキュリティ設定を開く」（S7）。
2. 「二要素認証」の「設定する」を押す。
3. 表示された QR コードを認証アプリで読み取る（読み取れない場合は表示された鍵を手入力）。
4. アプリに出た 6 桁のコードを入力して「確定」。
5. 解除するときは、同じ区画で現在のコードを入力して「無効にする」。

### パスキーを登録したいとき

1. セキュリティ（S7）を開く
   （**`localhost` などのドメイン名でアクセスしていること**）。
2. 名前（例: `Work laptop`）を入れて「パスキーを追加」。
3. ブラウザの案内に従って画面ロックまたはセキュリティキーで承認する。
4. 一覧に追加される。不要になったら一覧から削除する。

### IdP（SSO）と連携したいとき

パスワードで入っている口座を、そのまま IdP でも入れるようにする（ADR-0040）。

1. セキュリティ（S7）を開く。
2. 「シングルサインオン」の「〇〇 と連携する」を押す。
3. IdP の画面で認証する（既に IdP に入っていれば、そのまま戻ってくる）。
4. この画面に戻り、「連携しました」と出る。以後はログイン画面の「SSO で入る」
   からも入れる。

⚠ **途中でサインアウトすると完了しない**（結び付ける相手が決められないため）。
入り直してからもう一度押す。

解除は同じ区画の「連携を解除する」。⚠ **これが唯一の入り口になっている場合は
解除できない** ——先にパスワードを設定するか、パスキーを登録する。

### ユーザーを追加したいとき（`user:manage`）

1. サイドバーの「ユーザー」を開く。
2. 見出しの横の「ユーザーを追加」を押す（スマートフォンでは画面の下から
   幅いっぱいのシートが出る）。
3. メール・ユーザー名・パスワードを入力し、割り当てるロールを選んで「追加」。
   **複数選べる**（例: 普段は `member`、必要なときだけ `admin`）。
   やめるときは「キャンセル」・✕ ・Esc ・背景のタップのいずれか。
4. 後から変えるときは一覧のロール列のチェックを付け外しする。
5. 一時的に止めるときは一覧の「有効」を切り替える（削除ではなく無効化を推奨）。

### 操作するロールを切り替えたいとき

1. ヘッダーの「操作中のロール」を押す（**2 つ以上のロールを持つ人にだけ出る**）。
2. 使いたいロールを選ぶ。「すべてのロール」を選ぶと、持っている権限すべてに戻る。
3. 切り替えた瞬間から、サイドバーの項目も操作できる範囲もそのロールのものになる。

> 権限は**減る方向にしか**動かない（持っていないロールへは切り替えられない）。
> 「member から画面がどう見えるか」を確かめたいときや、普段は弱いロールで
> 操作したいときに使う。切り替えは監査ログに `role.switched` として残る。
> 選択はそのブラウザのサインインに紐づく（他の端末や次回のサインインには
> 引き継がれず、「すべてのロール」から始まる）。

### 権限を変えたいとき（`role:manage`）

1. サイドバーの「ロール」を開く。
2. 対象ロールの権限（scope）のチェックを変更する。ユーザー個別ではなく
   **ロールに対して**付与する。列見出し（権限コード）は縦向きに書かれており、
   ロール名の列は横になぞっても左端に残る。
3. 新しいロールが必要なら「ロールを追加」で名前を入れて「追加」し、
   表のチェックで権限を割り当てる（**追加した直後は権限を持たない**）。

> 有効 scope はユーザーが持つ全ロールの権限の和集合（本人がロールを 1 つに
> 絞っているあいだはそのロール分だけ。ADR-0017）。反映にはユーザーの
> 再サインイン（トークン再発行）が必要。

### システム設定を変えたいとき（`admin:system-settings`）

1. サイドバーの「システム設定」を開く。
2. 値を編集して「保存」。
3. 「再起動後に反映」と出た項目を変更した場合は「今すぐ再起動」を押す
   （`system:manage` が必要）。数秒後にアプリが復帰する。

> 「環境変数で固定」と表示されている項目は画面から変更できない。
> 優先順位は 環境変数 > DB > デフォルト値。

### エラーを調べたいとき（`log:view`）

1. サイドバーの「システムログ」を開く。
2. レベルに `ERROR` を選んで「検索」。期間を絞るなら開始・終了（**UTC**）も入れる。
3. 例外の行は「traceback を表示」で詳細を開く。
4. 原因を追うときは、その行の `requestId` を控えて手順を変える。
   - **同じリクエストの他のログ**を見る → `requestId` 欄に貼って「検索」。
   - **誰の操作だったか**を見る → 「監査ログ」画面の `requestId` 欄に貼って「検索」。

特定のモジュールだけ見たいときは「ロガー」に前方一致で入れる。

| ロガー                                | 出るもの                                                          |
| ------------------------------------- | ----------------------------------------------------------------- |
| `app.request`                         | HTTP アクセスログ（1 リクエスト 1 行）                            |
| `presentation.fastapi.error_handling` | エラー応答の記録（エラーコード・入力検証で落ちた項目・traceback） |
| `bounded_contexts`                    | 業務処理                                                          |

失敗だけを見たいときはレベルで絞る。**5xx は ERROR、4xx は WARNING**（401 は
期限切れ・未ログインで日常的に起きるため INFO）。

> `/healthz`・`/readyz`・`/api/health`・`/metrics` の**成功した**アクセスは
> この画面に出ない（死活監視の定期アクセスで埋まるため）。失敗したときは出る。

> 画面に出ないログがある場合は、システム設定の `LOG_DB_MIN_LEVEL` が DB への
> 書き込みを間引いている。stdout 側（コンテナのログ）には出ている。

### 誰が何をしたかを調べたいとき（`audit:view`）

1. サイドバーの「監査ログ」を開く。
2. よく使う絞り込み:
   - **不審なログイン試行** → 「失敗だけ表示」を押す。イベントが `login.failed` の行の
     「詳細」に理由（`unknown_email` / `invalid_password` / `invalid_totp` 等）が出る。
   - **あるユーザーへの操作** → 「対象の種別」に `user`、「対象 ID」にユーザー ID を入れる。
     そのアカウントを狙ったログイン失敗・パスワードリセット要求もここに出る。
   - **ある管理者の操作** → 「実行者（ユーザー ID）」にその管理者のユーザー ID を入れる。
   - **設定を変えたのは誰か** → イベントに `system_settings.updated` を選ぶ。
     「詳細」に変更されたキー名が出る（値は出ない）。
3. ユーザー ID から人を特定するときは「ユーザー」画面（`user:manage` が必要）の
   一覧と突き合わせる。

> 監査ログには個人情報を保存しない。実行者・対象は内部 ID で表示され、パスワードや
> 設定値そのものは記録されない。

### メールアドレス・表示名を変えたいとき

1. ヘッダーのユーザー名 → プロフィールを開く。
2. 「アカウント」のメールアドレス・ユーザー名を編集して「保存」。
3. 他のユーザーが使用中のメールアドレスは登録できない
   （「このメールアドレスは既に登録されています。」）。

> 保存後も再サインインは不要。次回のサインインから新しいメールアドレスを使う。

### 表示言語・テーマを変えたいとき

1. ヘッダーのユーザー名 → プロフィールを開く。
2. 「表示設定」の言語セレクタ・テーマセレクタで切り替える（即時反映。選択は
   ブラウザに保存され、端末ごとに独立する）。

テーマは ライト / ダーク / OS 追従 の 3 つ。既定値はシステム設定の
`DEFAULT_LOCALE` / `DEFAULT_THEME`。

### システムの状態を確認したいとき（`system:manage`）

1. サイドバーの「システム管理」→「システムステータス」を開く。
2. API・データベースの状態（正常/異常）と稼働時間、バージョン・git SHA・
   ビルド日時を確認する。「再読み込み」で取り直す。

### 「新しい版があります」と出たとき

1. 区切りのよいところで「再読み込み」を押す（書きかけの入力は失われる）。
2. 画面が読み込み直され、新しい版になる。
3. 押さずに使い続けてもよい。そのあいだは古い版のままで、知らせは消えない。

### スマートフォンで画面を切り替えたいとき

1. 左上のメニューボタン（☰）を押す。
2. 移動したい項目を選ぶ（選ぶとメニューは自動で閉じる）。
   - 閉じるだけなら ✕ ・メニューの外側をタップ・Esc のいずれか。
3. 表が画面幅に収まらないときは、**表の上を横になぞる**（表だけが横に動く）。
   ユーザー名・ロール名の列は左端に残るので、どの行を触っているかは分かる。
4. 追加の入力は画面の下から出るシートで行う。決定と取り消しはシートの下端に並ぶ。
5. ログの絞り込みは畳まれている。条件を変えるときは「絞り込み」を押して開く。

---

## ディレクトリ構成

```
src/
  App.tsx           # ルート定義と RequireAuth（画面を増やすときはここも）
  components/       # AppLayout（外枠・ドロワー）/ Header / Sidebar / Footer /
                    # RoleSwitcher（ロールの切り替え）/
                    # PreferenceControls（言語・テーマ）/ ToastNotification /
                    # FormDialog（追加フォームのダイアログ）/
                    # FilterPanel（ログの絞り込みの開閉）/
                    # AppUpdatePrompt（新しい版の知らせ）
  pages/            # 画面 1 つ = 1 ファイル（*Page.tsx）
  services/         # api.ts（fetch + トークン更新）/ webauthn.ts / uiSettings.ts /
                    # appUpdate.ts（新しい版の見張り）
  store/            # AuthContext（サインイン状態・scope 判定）
  i18n/             # 言語別 JSON（英語キーで定義し各言語へ訳す）
  theme/            # テーマ適用
  test-support/     # テスト用ヘルパー
```

新しい画面を追加するときの手順:

1. `src/pages/<Name>Page.tsx` を作る。
2. `src/App.tsx` にルートを追加する（認証が必要なら `RequireAuth` の中）。
3. サイドバーに出すなら `components/Sidebar.tsx` の `TOP_ITEMS`（最上位）か
   `GROUPS`（管理／システム管理）へ **必要な scope 付きで**追加する。
   表を置くときは `<div className="table-scroll">` で包む（狭い画面で
   ページ全体が横スクロールしないようにするため）。**追加フォームは画面に直接
   置かず、「追加」ボタン + `FormDialog` にする**（ADR-0027）。
4. 文言は `src/i18n/en.json` に英語キーで追加し、各言語ファイルへ訳を入れる。
5. **本 README の画面遷移図・画面一覧・画面仕様・操作マニュアルを更新する。**
6. `npm run test` と `make check-frontend` を通す。
