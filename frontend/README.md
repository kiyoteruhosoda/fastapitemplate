...
- **表示内容**: 追加フォーム（メール・ユーザー名・パスワード・ロールの複数選択）と、
  一覧（**ロールごとに 1 列**のチェックボックス、有効フラグ、削除）。
- **操作**:
  1. 追加フォームで必要なロールを**いくつでも**選んで「追加」。
...
- **備考**:
  ...
  - 狭い画面での扱いは S12 と同じ（絞り込みは 1 列、表だけが横スクロールする）。
...
1. サイドバーの「ユーザー」を開く。
2. メール・ユーザー名・パスワードを入力し、割り当てるロールを選んで「追加」。
   **複数選べる**（例: 普段は `member`、必要なときだけ `admin`）。
3. 後から変えるときは一覧のロール列のチェックを付け外しする。
4. 一時的に止めるときは一覧の「有効」を切り替える（削除ではなく無効化を推奨）。
