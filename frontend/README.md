...
flowchart TD
    ...
    subgraph private["認証済み（Header + Sidebar + Footer レイアウト）"]
        ...
        Items["/items<br/>アイテム"]
        Profile["/profile<br/>プロフィール<br/>（パスワード・2FA・パスキー・表示設定）"]
...
- 未定義のパス（`*`）は `/` にリダイレクトされる。
- 旧 URL の `/change-password`・`/security` は `/profile` へ転送される（ADR-0020）。
...
| S7  | ユーザー管理           | `/admin/users`            | 必要 | `user:manage`                                       | ✅         |
| S8  | ロール管理             | `/admin/roles`            | 必要 | `role:manage`（一覧の取得のみ `user:manage` も可）  | ✅         |
| S9  | 権限一覧               | `/admin/permissions`      | 必要 | `permission:manage`                                 | ✅         |
| S10 | システム設定           | `/admin/config`           | 必要 | `admin:system-settings`（再起動は `system:manage`） | ✅         |
| S11 | システムログ           | `/admin/logs`             | 必要 | `log:view`                                          | ✅         |
| S12 | 監査ログ               | `/admin/audit-logs`       | 必要 | `audit:view`                                        | ✅         |
| S13 | システムステータス     | `/admin/system-status`    | 必要 | `system:manage`                                     | ✅         |

サイドバーの階層: S4・S5 は最上位、S7〜S9 は「管理」、S10〜S13 は「システム管理」の
グループに入る。
...
欄ごとに独立し、画面を離れると元に戻る（保存しない）。対象は S1・S3・S6・S7 の
パスワード欄と、S10 の秘匿項目（`MAIL_PASSWORD` 等）。
...
- **目的**: サインイン中のユーザー自身のアカウント情報・サインインの手段
  （パスワード・二要素認証・パスキー）・表示設定（言語・テーマ）を 1 つの画面で
  確認・変更する（ADR-0016 / ADR-0020）。
- **表示内容**: 「アカウント」（メールアドレス・ユーザー名の編集フォーム）、
  「パスワード変更」（現在のパスワード・新しいパスワード）、「二要素認証」
  （有効・無効の状態、登録時の QR コードと手入力用の鍵）、「パスキー」（追加フォームと
  登録済みの一覧: 名前・登録日時・最終使用日時）、「表示設定」（言語・テーマの
  セレクタ）、「ロール」（割り当てられているロール。操作中のものに「使用中」が付く）、
  「有効なスコープ」（**いま**使える scope。ロールを絞っているあいだはその範囲だけ）。
- **操作**:
  1. メールアドレス・ユーザー名を編集して「保存」。重複するメールアドレスは
     `error.email_already_exists` で拒否される。
  2. 現在のパスワードと新しいパスワードを入力して「変更」。
  3. 二要素認証: 「設定する」→ QR を認証アプリで読み取り → コードを入力して確定。
     解除は現在のコードの入力が必要。
  4. パスキー: 名前を入れて「パスキーを追加」→ ブラウザの認証を実行 → 登録。
     一覧から削除。
  5. 言語・テーマを切り替える（即時反映。選択はブラウザに保存される）。
...
- **備考**:
  - メールアドレスを変えても再サインインは不要（トークンはユーザー ID で引き直す）。
    次回のサインインから新しいメールアドレスを使う。変更は監査ログに
    `profile.updated` として残る（変更した項目名のみ。値は記録しない）。
  - ブラウザが WebAuthn 非対応のときは「パスキー」に非対応メッセージを出す。

### S7 ユーザー管理（`/admin/users`）
...
- **備考**:
  ...
  - 狭い画面での扱いは S11 と同じ（絞り込みは 1 列、表だけが横スクロールする）。

### S13 システムステータス（`/admin/system-status`）
...
1. ヘッダーのユーザー名 → プロフィール（S6）の「パスワード変更」。
2. 現在のパスワードと新しいパスワードを入力して「変更」。
...
1. ヘッダーのユーザー名 → プロフィール（S6）の「二要素認証」。
2. 「設定する」を押す。
