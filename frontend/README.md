...
| S15 | システムステータス     | `/admin/system-status`    | 必要 | `system:manage`                                     | ✅         |

サイドバーの階層: S4・S5 は最上位、S9〜S11 は「管理」、S12〜S15 は「システム管理」の
グループに入る。
...
欄ごとに独立し、画面を離れると元に戻る（保存しない）。対象は S1・S3・S7・S9 の
パスワード欄と、S12 の秘匿項目（`MAIL_PASSWORD` 等）。
...
### S15 システムステータス（`/admin/system-status`）
...
- **Sidebar**: S4〜S15 のうち、**保有 scope に合致する項目だけ**を表示する。
