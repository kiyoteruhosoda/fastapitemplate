...
- 旧名（テンプレート名）で動いていた環境からの移行として、旧アプリ名の compose
  ...
  `DOCKER_NETWORK_NAME` / `HOST_DATA_ROOT`）を新しい名前・現在地へ書き換える。
  `.env` は旧既定値と**完全一致する行だけ**を書き換える（運用者が選んだ値には触れない）。
  永続データはホスト側の `HOST_DATA_ROOT` にあるため消えない。
  - 畳むのは、そのプロジェクトのコンテナが `com.docker.compose.project.working_dir`
    ラベルでこの環境ディレクトリと一致するものだけで構成されているときに限る
    （プロジェクト名ラベルだけを根拠にすると、同じ旧名を使う別のアプリを停止・削除して
    しまうため）。
  - ディレクトリごと改名して移行した場合、旧コンテナのラベルは移動前のパスを指したまま
    残る。移動前のパス（`<親の親>/fastapitemplate/<env>`）とも突き合わせるが、**そこに
    まだディレクトリがある場合は別のデプロイが現役でいるとみなして畳まない**。
  - `HOST_DATA_ROOT` を直すのも、移動前のパス（`<移動前の環境ディレクトリ>/mnt`）ちょうどを
    指していて、そこが実在しないときだけ。別ディスク・共有フォルダを指している値には
    触れない（一時的に見えていないだけの場合に、空の DB で起動してしまうため）。
